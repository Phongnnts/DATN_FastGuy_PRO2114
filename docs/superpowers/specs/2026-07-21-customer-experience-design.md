...
- `/` la login; public home la `/home`.
- Guest PayOS return gui `orderCode` va opaque `returnProof`; backend chi luu hash va doc lai payment status. Browser return khong tu xac nhan thanh toan; cart chi clear khi backend bao thanh cong, co recovery khi cancel/timeout.
