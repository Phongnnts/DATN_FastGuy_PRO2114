...
- Cancellation truoc consume ghi `RELEASE`; cancellation tu `PREPARING` tro di sau consume ghi `WASTE`, khong restock tu dong.
- Inventory transaction la immutable ledger: `RESERVE`, `RELEASE`, `CONSUME`, `WASTE`, `RETURN`, `ADJUSTMENT`.

## Payment And Refund

- PayOS capability gom tao checkout link, truy van provider status va signed webhook; can du ba bien `PAYOS_*`.
- `PaymentAttempt` luu provider reference, amount, status, expiry va dedupe key.
- Signed webhook/backend verification la source of truth; browser return khong xac nhan thanh toan. Guest chi doc payment status bang `orderCode` va opaque `returnProof`; server chi luu hash proof.
