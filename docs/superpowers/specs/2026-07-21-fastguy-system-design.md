...
- Huy truoc `PREPARING` ghi `RELEASE`; huy tu `PREPARING` tro di ghi inventory transaction `WASTE`, khong tu dong cong hang da che bien vao ton ban.
...
- PayOS ho tro checkout link, provider status va signed webhook khi du cau hinh `PAYOS_*`; webhook/backend verification la nguon thanh toan chuan, browser return khong phai proof.
- Guest payment return can `orderCode` va opaque `returnProof`; backend chi luu hash, event trung lap khong tao side effect.
