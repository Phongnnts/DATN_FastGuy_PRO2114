...
## Pre-requisites
- [ ] Fresh/reset database bằng `sqlcmd -S localhost -E -C -b -i database/init.sql` (script xóa và tạo lại `FastGuyDB`)
...
### 5.2 Delivery Flow
- [ ] Pickup đơn ASSIGNED cho đúng shipper đang `CHECKED_IN` → status = PICKED_UP
