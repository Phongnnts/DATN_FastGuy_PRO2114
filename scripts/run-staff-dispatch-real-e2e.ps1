...
function Invoke-Fixture([string]$Action, [string]$RunId) {
    ...
    $fixture = if ($Operations -or $NavigationR1 -or $DashboardR3) { 'integration.OperationsBrowserFixtureIT' } elseif ($Ownership) { 'integration.StaffOwnershipBrowserFixtureIT' } else { 'integration.StaffDispatchBrowserFixtureIT' }
...
try {
    $env:FASTGUY_E2E_DB_NAME = if ($Operations -or $NavigationR1 -or $DashboardR3) { 'FastGuyDB_Operations060_Test' } else { 'FastGuyDB_Inventory054_Test' }
...
"@ | Set-Content -LiteralPath $serverXml -Encoding UTF8
    ...
    foreach ($project in $projects) {
        ...
        if ($Operations -or $NavigationR1 -or $DashboardR3) {
