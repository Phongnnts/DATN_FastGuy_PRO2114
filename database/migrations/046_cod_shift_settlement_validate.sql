...
BEGIN TRY
    ...
    IF EXISTS (
        SELECT required.name FROM (VALUES
            ...
            (N'reason', N'nvarchar', 1000, 0, 0, 1), (N'submitted_at', N'datetime2', 6, 19, 0, 0), (N'verified_at', N'datetime2', 6, 19, 0, 1),
            (N'created_at', N'datetime2', 6, 19, 0, 0), (N'updated_at', N'datetime2', 6, 19, 0, 0)
    ...
    IF EXISTS (
        SELECT required.name FROM (VALUES
            (N'CK_CodSettlement_Status', N'SUBMITTED'), (N'CK_CodSettlement_Status', N'SETTLED'),
            (N'CK_CodSettlement_Status', N'SHORT'), (N'CK_CodSettlement_Status', N'OVER'),
            (N'CK_CodSettlement_Amounts', N'EXPECTED_AMOUNT'), (N'CK_CodSettlement_Amounts', N'SUBMITTED_AMOUNT'),
            (N'CK_CodSettlement_Amounts', N'VERIFIED_AMOUNT'),
            (N'CK_CodSettlement_Verification', N'RECEIVED_BY'), (N'CK_CodSettlement_Verification', N'VERIFIED_AMOUNT'),
            (N'CK_CodSettlement_Verification', N'SUBMITTED_AMOUNT'), (N'CK_CodSettlement_Verification', N'REASON'),
            (N'CK_CodSettlement_Verification', N'VERIFIED_AT')
        ) required(name, token)
        LEFT JOIN sys.check_constraints cc ON cc.parent_object_id = OBJECT_ID(N'dbo.CodSettlement') AND cc.name = required.name
        WHERE cc.object_id IS NULL OR cc.is_disabled = 1 OR cc.is_not_trusted = 1 OR CHARINDEX(required.token, UPPER(cc.definition)) = 0
