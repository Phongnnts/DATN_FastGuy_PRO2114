...
public class OperatingExpense {
    public enum Category { RENT, UTILITIES, SALARY, MARKETING, MAINTENANCE, OTHER }

    @Id @GeneratedValue(strategy = GenerationType.IDENTITY) @Column(name = "expense_id", nullable = false)
    private int expenseId;
    @Column(name = "expense_date", nullable = false) private LocalDate expenseDate;
    @Enumerated(EnumType.STRING) @Column(name = "category", nullable = false, length = 20) private Category category;
    ...
    public int getExpenseId(){return expenseId;} public void setExpenseId(int v){expenseId=v;}
