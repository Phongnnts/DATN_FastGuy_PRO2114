...
public class OrderItem {
    ...
    public void setTotalCostSnapshot(BigDecimal value) {
        totalCostSnapshot = value;
    }

    public List<ModifierItem> getModifiers() {
        if (
            modifiersJson == null || modifiersJson.isEmpty()
        ...
        try {
            this.modifiersJson =
                modifiers == null || modifiers.isEmpty()
                    ? "[]"
                    : utils.JsonUtil.getMapper().writeValueAsString(modifiers);
        } catch (Exception e) {
            this.modifiersJson = "[]";
        }
    }
