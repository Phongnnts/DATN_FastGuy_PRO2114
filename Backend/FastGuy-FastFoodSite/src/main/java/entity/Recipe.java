...
public class Recipe {
    ...
    @PrePersist
    void prePersist() {
        createdAt = updatedAt = LocalDateTime.now().withNano(0);
    ...
    public void setYieldQuantity(BigDecimal quantity) {
        if (
            quantity == null || quantity.compareTo(BigDecimal.ZERO) <= 0
        ) throw new IllegalStateException("Invalid yield quantity");
