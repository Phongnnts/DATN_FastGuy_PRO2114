...
@Check(constraints = "quantity <> 0 AND transaction_type IN ('RECEIPT', 'RESERVE', 'RELEASE', 'CONSUME', 'ADJUSTMENT', 'WASTE', 'RETURN')")
public class InventoryTransaction {
    @Id @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "inventory_transaction_id") private int inventoryTransactionId;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "inventory_item_id", nullable = false) private InventoryItem inventoryItem;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "order_id") private Orders order;
    ...
    @Column(name = "quantity_before", precision = 19, scale = 4) private BigDecimal quantityBefore;
    @Column(name = "quantity_after", precision = 19, scale = 4) private BigDecimal quantityAfter;
    ...
    @Column(name = "total_cost", precision = 19, scale = 4) private BigDecimal totalCost;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "goods_receipt_id") private GoodsReceipt goodsReceipt;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "stock_count_id") private StockCount stockCount;
    @Column(name = "created_at", nullable = false, columnDefinition = "datetime2(0)") private LocalDateTime createdAt;
    @PrePersist void prePersist() { createdAt = LocalDateTime.now().withNano(0); }
    ...
    public void setQuantity(BigDecimal quantity) { if (quantity == null || quantity.compareTo(BigDecimal.ZERO) == 0) throw new IllegalStateException("Invalid transaction quantity"); this.quantity = quantity; }
    ...
    public GoodsReceipt getGoodsReceipt(){return goodsReceipt;} public void setGoodsReceipt(GoodsReceipt value){goodsReceipt=value;}
    public StockCount getStockCount(){return stockCount;} public void setStockCount(StockCount value){stockCount=value;}
}
