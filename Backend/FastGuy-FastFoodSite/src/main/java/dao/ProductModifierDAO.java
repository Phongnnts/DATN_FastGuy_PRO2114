...
public class ProductModifierDAO {
    ...
    public boolean isOptionReferencedInOrders(int optionId) { EntityManager em = DatabaseUtil.getEntityManager(); try { Long count = em.createQuery("SELECT COUNT(m) FROM OrderItemModifier m WHERE m.option.modifierOptionId = :id", Long.class).setParameter("id", optionId).getSingleResult(); return count > 0; } finally { em.close(); } }
    public void deleteGroupWithOptions(int groupId) { transact(em -> { ProductModifierGroup g = em.find(ProductModifierGroup.class, groupId); if (g == null) return; List<ProductModifierOption> opts = em.createQuery("SELECT o FROM ProductModifierOption o WHERE o.group.modifierGroupId = :id", ProductModifierOption.class).setParameter("id", groupId).getResultList(); for (ProductModifierOption o : opts) em.remove(o); em.remove(g); }); }
    public void deleteOption(int optionId) { transact(em -> { if (isOptionReferencedInOrders(optionId)) throw new IllegalStateException("Option " + optionId + " is referenced in orders"); ProductModifierOption o = em.find(ProductModifierOption.class, optionId); if (o != null) em.remove(o); }); }
    public void deleteComboItem(int comboItemId) { transact(em -> { ProductComboItem i = em.find(ProductComboItem.class, comboItemId); if (i != null) em.remove(i); }); }
    public int countOptionsInGroup(int groupId) { EntityManager em = DatabaseUtil.getEntityManager(); try { return em.createQuery("SELECT COUNT(o) FROM ProductModifierOption o WHERE o.group.modifierGroupId = :id", Long.class).setParameter("id", groupId).getSingleResult().intValue(); } finally { em.close(); } }
