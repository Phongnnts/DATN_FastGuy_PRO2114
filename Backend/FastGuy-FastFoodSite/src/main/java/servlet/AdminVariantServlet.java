...
public class AdminVariantServlet extends HttpServlet {
    ...
    private Integer readStock(Map<String, Object> body) {
        ...
        if (
            !(body.get("quantityAvailable") instanceof Number)
        ) throw new IllegalArgumentException(
            "quantityAvailable must be an integer"
        );
        try {
            int value = new BigDecimal(
                body.get("quantityAvailable").toString()
            ).intValueExact();
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (path == null || path.equals("/")) {
            ApiResponse.error(resp, "Missing variant ID", 400);
            return;
        }
        try {
            ...
            if (v == null) {
                ApiResponse.error(resp, "Variant not found", 404);
                return;
            }
            ...
            if (body.containsKey("weight")) v.setWeight(
                BigDecimal.valueOf(((Number) body.get("weight")).doubleValue())
            );
            if (body.containsKey("length")) v.setLength(
                BigDecimal.valueOf(((Number) body.get("length")).doubleValue())
            );
            if (body.containsKey("width")) v.setWidth(
                BigDecimal.valueOf(((Number) body.get("width")).doubleValue())
            );
            if (body.containsKey("height")) v.setHeight(
                BigDecimal.valueOf(((Number) body.get("height")).doubleValue())
            );
