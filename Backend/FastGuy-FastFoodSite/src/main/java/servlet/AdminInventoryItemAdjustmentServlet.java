...
public class AdminInventoryItemAdjustmentServlet extends HttpServlet {
    ...
    public AdminInventoryItemAdjustmentServlet() {
        this(new AdminInventoryService());
    }

    AdminInventoryItemAdjustmentServlet(AdminInventoryService service) {
        this.service = service;
    }

    protected void doPost(HttpServletRequest q, HttpServletResponse p)
        throws IOException {
        int uid = requireAdmin(q, p);
        if (uid < 0) return;
        try {
            Map<String, Object> b = AdminInventoryItemServlet.body(q);
            AdminInventoryItemServlet.requireKeys(
                b,
                Set.of(
                    "inventoryItemId",
                    "quantity",
            ...
            int id = AdminInventoryItemServlet.positiveInt(
                b.get("inventoryItemId"),
                "inventoryItemId"
            );
            BigDecimal quantity = signedDecimal(b.get("quantity"));
            BigDecimal expected = AdminInventoryItemServlet.dec(
                b,
                "expectedOnHandQuantity",
                false
            );
            ApiResponse.ok(
                ...
                service.mutate(
                    ...
                    quantity,
                    expected,
                    AdminInventoryItemServlet.str(b, "reason"),
                    note(b),
                    uid
                )
            );
        } catch (InventoryItemConflictException e) {
            AdminInventoryItemServlet.conflict(p, e);
    ...
    protected int requireAdmin(HttpServletRequest q, HttpServletResponse p)
        throws IOException {
    ...
    private static BigDecimal signedDecimal(Object value) {
        try {
            BigDecimal d = new BigDecimal(String.valueOf(value));
            if (
                d.stripTrailingZeros().scale() > 4 ||
                d.signum() == 0 ||
                d.abs().compareTo(new BigDecimal("999999999999999.9999")) > 0
    ...
    private static String note(Map<String, Object> b) {
        Object n = b.get("note");
        if (n == null) return null;
        if (
            !(n instanceof String s) || s.length() > 500
