...
public class OperatingExpenseServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest q,HttpServletResponse p)throws IOException{int user=admin(q,p);if(user<0)return;try{String path=q.getPathInfo();if(path==null||"/".equals(path)){String from=q.getParameter("fromDate"),to=q.getParameter("toDate");if((from==null)!=(to==null))throw new IllegalArgumentException("fromDate and toDate must be provided together");if(from==null)ApiResponse.ok(p,service.listExpenses());else{LocalDate fromDate=LocalDate.parse(from),toDate=LocalDate.parse(to);if(fromDate.isAfter(toDate))throw new IllegalArgumentException("Invalid date range");ApiResponse.ok(p,service.listExpenses(fromDate,toDate));}}else ApiResponse.ok(p,service.getExpense(id(path)));}catch(NoSuchElementException e){ApiResponse.error(p,e.getMessage(),404);}catch(IllegalArgumentException|DateTimeException e){ApiResponse.error(p,e.getMessage(),400);}catch(RuntimeException e){ApiResponse.error(p,"Internal server error",500);}}
