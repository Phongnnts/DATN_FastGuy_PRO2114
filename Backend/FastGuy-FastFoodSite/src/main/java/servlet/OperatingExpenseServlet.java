...
public class OperatingExpenseServlet extends HttpServlet {
    ...
    public OperatingExpenseServlet() {
        this(new OperatingFinanceService());
    }

    OperatingExpenseServlet(OperatingFinanceService service) {
    ...
    protected void doGet(HttpServletRequest q, HttpServletResponse p)
        ...
        try {
            ...
            if (path == null || "/".equals(path)) {
                ...
                if (from == null) ApiResponse.ok(p, service.listExpenses());
                else {
                    ...
                    ApiResponse.ok(p, service.listExpenses(fromDate, toDate));
                }
            } else ApiResponse.ok(p, service.getExpense(id(path)));
    ...
    protected void doDelete(HttpServletRequest q, HttpServletResponse p)
        ...
        if (user < 0) return;
        try {
            service.deleteExpense(id(q.getPathInfo()));
            p.setStatus(204);
        } catch (NoSuchElementException e) {
            ApiResponse.error(p, e.getMessage(), 404);
    ...
    static Map<String, Object> body(HttpServletRequest q) throws IOException {
        try {
            Map<String, Object> b = utils.JsonUtil.getMapper().readValue(
                q.getReader(),
                Map.class
            );
    ...
    static void exact(Map<String, Object> b, Set<String> keys) {
        if (!b.keySet().equals(keys)) throw new IllegalArgumentException(
            keys.containsAll(b.keySet())
    ...
    static String str(Map<String, Object> b, String key) {
        if (
            !(b.get(key) instanceof String value)
        ) throw new IllegalArgumentException("Invalid " + key);
        return value;
    }

    static LocalDate date(Map<String, Object> b, String key) {
        return LocalDate.parse(str(b, key));
    }

    static BigDecimal money(Map<String, Object> b, String key) {
        if (
            !(b.get(key) instanceof Number value)
        ) throw new IllegalArgumentException("Invalid " + key);
        return new BigDecimal(value.toString());
    }

    static int integer(Map<String, Object> b, String key) {
        ...
        try {
            ...
            if (decimal.stripTrailingZeros().scale() > 0) return -1;
