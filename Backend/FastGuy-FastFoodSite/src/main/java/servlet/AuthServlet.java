...
public class AuthServlet extends HttpServlet {
    ...
    private void handleLogin(Map<String, Object> body, HttpServletResponse resp)
        throws IOException {
        ...
        if (login == null || password == null) {
            JsonUtil.write(
                resp,
                ApiResponse.error("Vui lòng nhập tài khoản và mật khẩu")
            );
            return;
        }
        ...
        if (user == null) {
            JsonUtil.write(
                resp,
                ApiResponse.error("Sai tài khoản hoặc mật khẩu")
            );
            return;
        }
    ...
    private void handleRegister(
        ...
        String validationError = validateAccount(
            fullName,
            phone,
            email,
            password,
        ...
        if (user == null) {
            JsonUtil.write(
                resp,
                ApiResponse.error("Số điện thoại hoặc email đã tồn tại")
            );
            return;
        }
    ...
    private void handleLogout(HttpServletResponse resp) throws IOException {
        JsonUtil.write(
            resp,
            ApiResponse.ok((Object) null, "Đăng xuất thành công")
        );
    }
    ...
    private void handleResetPassword(
        ...
        try {
            ...
            JsonUtil.write(
                resp,
                ApiResponse.ok((Object) null, "Đặt lại mật khẩu thành công")
            );
    ...
    private void handleChangePassword(
        ...
        if (
            ...
            JsonUtil.write(
                resp,
                ApiResponse.error("Vui lòng nhập đầy đủ mật khẩu")
            );
            return;
        }
        ...
        try {
            ...
            JsonUtil.write(
                resp,
                ApiResponse.ok((Object) null, "Đổi mật khẩu thành công")
            );
        } catch (IllegalArgumentException e) {
            JsonUtil.write(resp, ApiResponse.error(e.getMessage()));
        }
    ...
    private void handleUpdateProfile(
        ...
        if (body.containsKey("avatarUrl")) {
            try {
                body.put(
                    "avatarUrl",
                    UserAvatarPolicy.normalize(body.get("avatarUrl"))
                );
            } catch (IllegalArgumentException e) {
                ApiResponse.error(resp, e.getMessage(), 400);
                return;
        ...
        JsonUtil.write(
            resp,
            ApiResponse.ok(toProfileMap(user), "Cập nhật thành công")
        );
    }
    ...
    private String validateAccount(
        ...
        if (
            fullName == null || fullName.length() < 2 || fullName.length() > 100
    ...
    private void handleCartMigrate(
        HttpServletRequest req,
        Map<String, Object> body,
        HttpServletResponse resp
    ) throws IOException {
        int userId = getUserIdFromToken(req);
        if (userId < 0) {
            JsonUtil.write(resp, ApiResponse.error("Unauthorized"));
            return;
        }

        List<Map<String, Object>> items = (List<Map<String, Object>>) body.get(
            "items"
        );
        if (items == null || items.isEmpty()) {
            ApiResponse.ok(resp, (Object) null);
            return;
        }
