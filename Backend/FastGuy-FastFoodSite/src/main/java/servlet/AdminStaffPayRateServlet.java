...
public class AdminStaffPayRateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest q, HttpServletResponse p)
        throws IOException {
        int admin = AdminApiAuth.require(q, p, AdminApiAuth.jwt());
        if (admin < 0) return;
        try {
            ApiResponse.ok(p, service.list(userId(q.getPathInfo())));
    ...
    protected void doPost(HttpServletRequest q, HttpServletResponse p)
        ...
        try {
            ...
            ) throw new IllegalArgumentException("Invalid pay rate payload");
            p.setStatus(201);
            ApiResponse.ok(
                p,
                service.create(
                    userId(q.getPathInfo()),
                    LocalDate.parse((String) b.get("effectiveFrom")),
                    decimal(b.get("regularHourlyRate")),
                    decimal(b.get("overtimeHourlyRate")),
                    admin
                )
            );
        } catch (StaffPayRateService.DuplicateRate e) {
    ...
    static int userId(String path) {
        if (
            path == null || !path.matches("/[1-9]\\d*/pay-rates")
