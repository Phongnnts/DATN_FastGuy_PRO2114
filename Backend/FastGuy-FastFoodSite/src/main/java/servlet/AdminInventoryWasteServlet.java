...
public class AdminInventoryWasteServlet extends HttpServlet {
    ...
    public AdminInventoryWasteServlet() {
        this(new AdminInventoryService());
    }

    AdminInventoryWasteServlet(AdminInventoryService service) {
        this.service = service;
    }

    protected void doPost(HttpServletRequest q, HttpServletResponse p)
        throws IOException {
        int uid = AdminApiAuth.require(q, p, AdminApiAuth.jwt());
        if (uid < 0) return;
        try {
            ...
            BigDecimal quantity = AdminInventoryItemServlet.dec(
                b,
                "quantity",
                true
            );
