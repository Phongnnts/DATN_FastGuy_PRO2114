...
public class StockCountServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest q,HttpServletResponse p)throws IOException{int user=admin(q,p);if(user<0)return;try{String info=q.getPathInfo();if(info==null||"/".equals(info)){Map<String,Object>b=AdminInventoryItemServlet.body(q);AdminInventoryItemServlet.requireExactKeys(b,Set.of("countDate","inventoryItemIds"));if(!(b.get("inventoryItemIds") instanceof List<?> rawIds))throw new IllegalArgumentException("Invalid inventoryItemIds");List<Integer>itemIds=rawIds.stream().map(value->AdminInventoryItemServlet.positiveInt(value,"inventoryItemId")).toList();Map<String,Object>result=service.create(LocalDate.parse(AdminInventoryItemServlet.str(b,"countDate")),itemIds,user);p.setStatus(201);ApiResponse.ok(p,result);}else{String[]parts=path(info);if(!"approve".equals(parts[1]))throw new IllegalArgumentException("Invalid path");ApiResponse.ok(p,service.approve(Integer.parseInt(parts[0]),user));}}catch(NoSuchElementException e){ApiResponse.error(p,e.getMessage(),404);}catch(IllegalStateException e){ApiResponse.error(p,e.getMessage(),409);}catch(IllegalArgumentException|java.time.DateTimeException e){ApiResponse.error(p,e.getMessage(),400);}catch(RuntimeException e){ApiResponse.error(p,"Internal server error",500);}}
