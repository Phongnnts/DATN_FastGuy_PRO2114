...
public class StockCountServlet extends HttpServlet {
    ...
    public StockCountServlet() {
        this(new StockCountService());
    }

    StockCountServlet(StockCountService value) {
        service = value;
    }
        ...
        try {
            ...
            if (info == null || "/".equals(info)) {
                ...
                AdminInventoryItemServlet.requireExactKeys(
                    b,
                    Set.of("countDate", "inventoryItemIds")
                );
                if (
                    !(b.get("inventoryItemIds") instanceof List<?> rawIds)
                ) throw new IllegalArgumentException(
                    "Invalid inventoryItemIds"
                );
                List<Integer> itemIds = rawIds
                    .stream()
                    .map(value ->
                ...
                Map<String, Object> result = service.create(
                    LocalDate.parse(
                        AdminInventoryItemServlet.str(b, "countDate")
                    ),
                    itemIds,
                    user
                );
    ...
    protected void doPut(HttpServletRequest q, HttpServletResponse p)
        ...
        try {
            ...
            Map<String, Object> b = AdminInventoryItemServlet.body(q);
            AdminInventoryItemServlet.requireExactKeys(b, Set.of("items"));
            if (
                !(b.get("items") instanceof List<?> items)
    ...
    static String[] path(String value) {
        return GoodsReceiptServlet.path(value);
    }
