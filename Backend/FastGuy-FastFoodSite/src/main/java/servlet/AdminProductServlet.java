...
public class AdminProductServlet extends HttpServlet {
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (segs.length == 3 && "modifier-groups".equals(segs[1]) && "options".equals(segs[2])) {
            Integer optionId = parseId(segs[0]);
            if (optionId == null) { resp.sendError(404); return; }
            ProductModifierOption opt = modifierDAO.option(optionId);
            if (opt == null) { ApiResponse.error(resp, "Not found", 404); return; }
            Map<String, Object> body = JsonUtil.fromJson(req.getReader(), Map.class);
            if (body == null) { ApiResponse.error(resp, "Invalid data", 400); return; }
            if (body.containsKey("name")) {
                String name = (String) body.get("name");
                if (name != null && !name.isBlank()) opt.setName(name.trim());
            }
            if (body.containsKey("price")) {
                try { opt.setPrice(readMoney(body, "price", opt.getPrice())); } catch (IllegalArgumentException e) { ApiResponse.error(resp, e.getMessage(), 400); return; }
            }
            if (body.containsKey("isActive")) opt.setIsActive(Boolean.TRUE.equals(body.get("isActive")));
            if (body.containsKey("sortOrder")) opt.setSortOrder(((Number) body.get("sortOrder")).intValue());
            modifierDAO.save(opt);
            Map<String, Object> updated = new HashMap<>();
            updated.put("modifierOptionId", opt.getModifierOptionId());
            updated.put("name", opt.getName());
            updated.put("price", opt.getPrice());
            updated.put("isActive", Boolean.TRUE.equals(opt.getIsActive()));
            ApiResponse.ok(resp, updated, "Updated");
            return;
        }

        if (segs.length == 2 && "modifier-groups".equals(segs[1])) {
            Integer groupId = parseId(segs[0]);
            if (groupId == null) { resp.sendError(404); return; }
            ProductModifierGroup group = modifierDAO.group(groupId);
            if (group == null) { ApiResponse.error(resp, "Not found", 404); return; }
            Map<String, Object> body = JsonUtil.fromJson(req.getReader(), Map.class);
            if (body == null) { ApiResponse.error(resp, "Invalid data", 400); return; }
            if (body.containsKey("name")) {
                String name = (String) body.get("name");
            ...
            if (body.containsKey("minSelections")) group.setMinSelections(((Number) body.get("minSelections")).intValue());
            if (body.containsKey("maxSelections")) group.setMaxSelections(((Number) body.get("maxSelections")).intValue());
            if (body.containsKey("isActive")) group.setIsActive(Boolean.TRUE.equals(body.get("isActive")));
            if (body.containsKey("sortOrder")) group.setSortOrder(((Number) body.get("sortOrder")).intValue());
            int min = group.getMinSelections();
            int max = group.getMaxSelections();
            if (min < 0 || max < min || max > 50) { ApiResponse.error(resp, "min/max selections không hợp lệ", 400); return; }
        ...
        if (segs.length == 2 && "combo".equals(segs[1])) {
            ...
            if (body.containsKey("isActive")) combo.setIsActive(Boolean.TRUE.equals(body.get("isActive")));
            modifierDAO.save(combo);
            ApiResponse.ok(resp, comboMap(combo), "Updated");
            return;
        }
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (segs.length == 2 && "modifier-groups".equals(segs[0])) {
            Integer gid = parseId(segs[1]);
            if (gid == null) { resp.sendError(404); return; }
            ProductModifierGroup group = modifierDAO.group(gid);
            if (group == null) { ApiResponse.error(resp, "Not found", 404); return; }
            boolean hasReferences = false;
            for (ProductModifierOption opt : modifierDAO.options(gid)) {
                if (modifierDAO.isOptionReferencedInOrders(opt.getModifierOptionId())) { hasReferences = true; break; }
            }
            if (hasReferences) {
                for (ProductModifierOption opt : modifierDAO.options(gid)) {
                    opt.setIsActive(false);
                    modifierDAO.save(opt);
                }
                group.setIsActive(false);
                modifierDAO.save(group);
                ApiResponse.ok(resp, null, "Disabled (referenced in orders)");
            } else {
                modifierDAO.deleteGroupWithOptions(gid);
                ApiResponse.ok(resp, null, "Deleted");
            }
            return;
        }
        ...
        if (segs.length == 4 && "modifier-groups".equals(segs[1]) && "options".equals(segs[2])) {
            Integer oid = parseId(segs[3]);
            if (oid == null) { resp.sendError(404); return; }
            ProductModifierOption opt = modifierDAO.option(oid);
            if (opt == null) { ApiResponse.error(resp, "Not found", 404); return; }
            if (modifierDAO.isOptionReferencedInOrders(oid)) {
                opt.setIsActive(false);
                modifierDAO.save(opt);
                ApiResponse.ok(resp, null, "Disabled (referenced in orders)");
            } else {
                modifierDAO.deleteOption(oid);
                ApiResponse.ok(resp, null, "Deleted");
            }
            return;
        }

        if (segs.length == 2 && "combo-items".equals(segs[0])) {
            Integer cid = parseId(segs[1]);
            if (cid == null) { resp.sendError(404); return; }
            modifierDAO.deleteComboItem(cid);
            ApiResponse.ok(resp, null, "Deleted");
            return;
        }
