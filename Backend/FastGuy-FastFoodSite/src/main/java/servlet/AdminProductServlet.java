...
public class AdminProductServlet extends HttpServlet {
    ...
    private Map<String, Object> toMap(Product p) {
        ...
        m.put(
            "variants",
            productDAO
                .findVariantsByProductId(p.getProductId())
                .stream()
        ...
        m.put(
            "modifierGroups",
            modifierDAO
                .groups(p.getProductId())
                .stream()
                .map(this::modifierGroupMap)
                .collect(Collectors.toList())
        );
    ...
    private Map<String, Object> modifierGroupMap(ProductModifierGroup group) {
        ...
        m.put(
            "options",
            modifierDAO
                .options(group.getModifierGroupId())
                .stream()
                .map(option -> {
                    Map<String, Object> o = new HashMap<>();
                    o.put("modifierOptionId", option.getModifierOptionId());
                    o.put("name", option.getName());
                    o.put("price", option.getPrice());
                    o.put(
                        "isActive",
    ...
    private BigDecimal readMoney(
        ...
        if (
            !(body.get(key) instanceof Number)
        ) throw new IllegalArgumentException(key + " must be a number");
        BigDecimal value = BigDecimal.valueOf(
            ((Number) body.get(key)).doubleValue()
        );
        if (
    ...
    private String readAuditString(Map<String, Object> body, String key) {
        if (
            !(body.get(key) instanceof String)
    ...
    private String readStatus(
        ...
        String status = body.containsKey(key)
            ? (String) body.get(key)
            : fallback;
        if (status == null) return fallback;
        if (
            !"AVAILABLE".equals(status) && !"UNAVAILABLE".equals(status)
    ...
    private LocalTime readTime(
        ...
        if (
            ...
            body.get(key) == null ||
            ((String) body.get(key)).isBlank()
        ) return body.containsKey(key) ? null : fallback;
        try {
            return LocalTime.parse((String) body.get(key));
        } catch (Exception e) {
            throw new IllegalArgumentException(key + " không hợp lệ");
        }
    }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (segs.length == 0) {
            List<Map<String, Object>> list = productDAO
                .findAll()
                .stream()
                .map(this::toMap)
                .collect(Collectors.toList());
        ...
        if (segs.length == 2 && "variants".equals(segs[1])) {
            ...
            List<Map<String, Object>> variants = productDAO
                .findVariantsByProductId(pid)
                .stream()
                .map(this::toVariantMap)
                .collect(Collectors.toList());
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (segs.length == 0) {
            ...
            Integer categoryId = readInteger(
                body,
                "categoryId",
                0,
                1,
            ...
            if (category == null) {
                ApiResponse.error(resp, "Category not found", 400);
                return;
            }
            ...
            if (body.containsKey("galleryImages")) {
                p.setGalleryImages(
                    toGalleryJson((List<String>) body.get("galleryImages"))
                );
            }
        ...
        if (segs.length == 2 && "modifier-groups".equals(segs[1])) {
            ...
            Product product =
                productId == null ? null : productDAO.findById(productId);
            String name = (String) body.get("name");
            if (product == null || name == null || name.isBlank()) {
                ApiResponse.error(resp, "Invalid modifier group", 400);
                return;
            }
            int min =
                body.get("minSelections") instanceof Number
                    ? ((Number) body.get("minSelections")).intValue()
                    : 0;
            int max =
                body.get("maxSelections") instanceof Number
                    ? ((Number) body.get("maxSelections")).intValue()
                    : 1;
            if (min < 0 || max < min) {
                ApiResponse.error(resp, "Invalid selection limits", 400);
                return;
            }
            ...
            resp.setStatus(201);
            ApiResponse.ok(resp, modifierGroupMap(group), "Created");
            return;
        }

        if (
            segs.length == 3 &&
            "modifier-groups".equals(segs[1]) &&
            "options".equals(segs[2])
        ) {
            Integer groupId = parseId(segs[0]);
            ProductModifierGroup group =
                groupId == null ? null : modifierDAO.group(groupId);
            String name = (String) body.get("name");
            if (group == null || name == null || name.isBlank()) {
                ApiResponse.error(resp, "Invalid modifier option", 400);
                return;
            }
            ...
            try {
                option.setPrice(readMoney(body, "price", BigDecimal.ZERO));
            } catch (IllegalArgumentException e) {
                ApiResponse.error(resp, e.getMessage(), 400);
                return;
            }
            ...
            resp.setStatus(201);
            ApiResponse.ok(resp, option, "Created");
            return;
        }
        ...
        if (segs.length == 2 && "variants".equals(segs[1])) {
            ...
            if (variantName == null || variantName.trim().isEmpty()) {
                ApiResponse.error(resp, "Tên biến thể không được trống", 400);
                return;
            }
            ...
            try {
                ...
                if (
                    body.containsKey("quantityAvailable")
                ) throw new IllegalArgumentException(
                    "Initial stock must be received through the inventory item API"
                );
    ...
    private Boolean readActive(Map<String, Object> body, Boolean fallback) {
        ...
        if (
            !(body.get("isActive") instanceof Boolean)
    ...
    static Boolean readBoolean(
        ...
        if (!body.containsKey(key)) return fallback;
        if (
            !(body.get(key) instanceof Boolean)
    ...
    static String readHomepageOccasion(
        Map<String, Object> body,
        String fallback
    ) {
        ...
        if (
            !(value instanceof String occasion) ||
            !List.of(
                "QUICK_BREAK",
                "OFFICE_LUNCH",
                "STUDENT",
    ...
    private static final Set<String> PRODUCT_UPDATE_FIELDS = Set.of(
        "categoryId",
        "name",
        "description",
        "basePrice",
    ...
    static void validateProductUpdate(Map<String, Object> body) {
        ...
        if (body.containsKey("categoryId")) readInteger(
            body,
            "categoryId",
            0,
            1,
        ...
        if (
            ...
            !(body.get("description") instanceof String)
        ) throw new IllegalArgumentException(
        ...
        if (
            ...
            (!(body.get("status") instanceof String status) ||
                !List.of("AVAILABLE", "UNAVAILABLE").contains(status))
        ) throw new IllegalArgumentException("status is invalid");
        validateTimeValue(body, "availableFrom");
        validateTimeValue(body, "availableTo");
        validateUrlValue(body, "imageUrl", true);
        if (body.containsKey("galleryImages")) {
            if (
                !(body.get("galleryImages") instanceof List<?> images)
            ) throw new IllegalArgumentException(
            ...
            for (Object image : images)
                validateUrl(
                    image,
                    false,
                    "galleryImages must contain valid URL strings"
                );
        }
        if (body.containsKey("isNew")) readBoolean(body, "isNew", false);
        if (body.containsKey("spiceLevel")) readInteger(
            body,
            "spiceLevel",
            0,
            0,
    ...
    static void validateProductCreate(Map<String, Object> body) {
        ...
        for (String field : List.of(
            ...
            if (!body.containsKey(field)) throw new IllegalArgumentException(
                field + " is required"
            );
        }
    }

    static void applyProductHomepageMetadata(
        Product product,
        Map<String, Object> body
    ) {
    ...
    private static void readMoneyValue(Map<String, Object> body, String key) {
        ...
        try {
            if (
                new BigDecimal(body.get(key).toString()).signum() < 0
            ) throw new IllegalArgumentException(key + " must be >= 0");
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException(key + " must be a number");
        }
    ...
    private static void validateTimeValue(
        Map<String, Object> body,
        String key
    ) {
        ...
        if (
            ...
            key + " must be a time string or null"
        );
        try {
    ...
    private static void validateUrlValue(
        Map<String, Object> body,
        String key,
        boolean nullable
    ) {
        ...
        validateUrl(
            body.get(key),
            true,
            key + " must be a valid URL string or null"
        );
    }
    ...
    private static void validateUrl(
        ...
        if (
            !(raw instanceof String value) || (!allowBlank && value.isBlank())
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (segs.length == 2 && "restore".equals(segs[1])) {
            Integer id = parseId(segs[0]);
            if (id == null || !productDAO.restore(id)) {
                ApiResponse.error(resp, "Product not found", 404);
                return;
            }
        ...
        if (segs.length == 2 && "modifier-groups".equals(segs[1])) {
            ...
            ProductModifierGroup group =
                groupId == null ? null : modifierDAO.group(groupId);
            if (group == null) {
                ApiResponse.error(resp, "Modifier group not found", 404);
                return;
            }
            try {
                if (
                    ...
                    (!(body.get("name") instanceof String) ||
                        ((String) body.get("name")).isBlank())
                ) throw new IllegalArgumentException(
                    "Invalid modifier group name"
                );
                if (body.containsKey("name")) group.setName(
                    ((String) body.get("name")).trim()
                );
                int min = readSelection(
                    body,
                    "minSelections",
                    group.getMinSelections()
                );
                int max = readSelection(
                    body,
                    "maxSelections",
                ...
                group.setMinSelections(min);
                group.setMaxSelections(max);
                group.setIsActive(readActive(body, group.getIsActive()));
                modifierDAO.save(group);
        ...
        if (
            ...
            Integer groupId = parseId(segs[0]);
            Integer optionId = parseId(segs[3]);
            ProductModifierOption option =
                optionId == null ? null : modifierDAO.option(optionId);
            if (
                groupId == null ||
                option == null ||
                option.getGroup().getModifierGroupId() != groupId
            ) {
                ApiResponse.error(resp, "Modifier option not found", 404);
                return;
            }
            try {
                if (
                    ...
                    "Invalid modifier option name"
                );
                if (body.containsKey("name")) option.setName(
                    ((String) body.get("name")).trim()
                );
                option.setPrice(readMoney(body, "price", option.getPrice()));
                option.setIsActive(readActive(body, option.getIsActive()));
                modifierDAO.save(option);
        ...
        if (segs.length == 1) {
            ...
            if (body.containsKey("categoryId")) {
                Category c = categoryDAO.findById(
                    readInteger(body, "categoryId", 0, 1, Integer.MAX_VALUE)
                );
                if (c == null) {
                    ApiResponse.error(resp, "Category not found", 404);
                    return;
                }
                p.setCategory(c);
            }
            if (body.containsKey("name")) p.setName(
                ((String) body.get("name")).trim()
            );
            if (body.containsKey("description")) p.setDescription(
                (String) body.get("description")
            );
            try {
                if (body.containsKey("basePrice")) p.setBasePrice(
                    readMoney(body, "basePrice", p.getBasePrice())
                );
                if (body.containsKey("status")) p.setStatus(
                    readStatus(body, "status", p.getStatus())
                );
                if (body.containsKey("availableFrom")) p.setAvailableFrom(
                    readTime(body, "availableFrom", p.getAvailableFrom())
                );
                if (body.containsKey("availableTo")) p.setAvailableTo(
                    readTime(body, "availableTo", p.getAvailableTo())
                );
                p.setIsNew(readBoolean(body, "isNew", p.getIsNew()));
                p.setSpiceLevel(
                    readInteger(body, "spiceLevel", p.getSpiceLevel(), 0, 3)
                );
            } catch (IllegalArgumentException e) {
                ApiResponse.error(resp, e.getMessage(), 400);
                return;
            }
            if (body.containsKey("imageUrl")) p.setImageUrl(
        ...
        if (segs.length == 2 && "variants".equals(segs[0])) {
            ...
            if (
                body.containsKey("quantityAvailable") ||
                body.containsKey("expectedQuantity") ||
                body.containsKey("reasonCode") ||
                body.containsKey("note")
            ) {
                ApiResponse.error(
                    resp,
                    "Variant inventory must be changed through the inventory item API",
                    400
                );
                return;
            }
            ...
            try {
                if (body.containsKey("price")) v.setPrice(
                    readMoney(body, "price", v.getPrice())
                );
                if (body.containsKey("originalPrice")) v.setOriginalPrice(
                    readMoney(body, "originalPrice", v.getOriginalPrice())
                );
                if (body.containsKey("status")) v.setStatus(
                    readStatus(body, "status", v.getStatus())
                );
            } catch (IllegalArgumentException e) {
                ApiResponse.error(resp, e.getMessage(), 400);
                return;
            ...
            if (body.containsKey("isDefault")) v.setIsDefault(
                (Boolean) body.get("isDefault")
            );
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (segs.length == 2 && "permanent".equals(segs[1])) {
            ...
            try {
                if (!productDAO.permanentlyDelete(id)) {
                    ApiResponse.error(resp, "Product not found", 404);
                    return;
                }
        ...
        if (segs.length == 2 && "modifier-groups".equals(segs[0])) {
            ...
            if (groupId == null || modifierDAO.group(groupId) == null) {
                ApiResponse.error(resp, "Modifier group not found", 404);
                return;
            }
