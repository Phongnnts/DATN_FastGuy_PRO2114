...
final class AdminApiAuth {
    ...
    static int require(HttpServletRequest req,HttpServletResponse resp,TokenReader reader)throws IOException{
        ...
        try{String token=header.substring(7);String role=reader.role(token);int userId=reader.userId(token);if(!"ADMIN".equals(role)||!reader.isActiveRole(userId,"ADMIN")){ApiResponse.error(resp,"Forbidden",403);return-1;}return userId;}
