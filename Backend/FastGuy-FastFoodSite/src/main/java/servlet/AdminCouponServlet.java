...
public class AdminCouponServlet extends HttpServlet {
    ...
    private Map<String, Object> toMap(Coupon c) {
        ...
        m.put(
            "minOrder",
            c.getMinOrder() != null ? c.getMinOrder() : BigDecimal.ZERO
        );
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (path == null || path.equals("/")) {
            List<Map<String, Object>> list = couponDAO
                .findAll()
                .stream()
                .map(this::toMap)
        ...
        try {
            Coupon coupon = couponDAO.findById(
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if (!oldCode.equals(coupon.getCode())) {
                ...
                if (
                    existing != null &&
                    existing.getCouponId() != coupon.getCouponId()
                ) {
                    ApiResponse.error(resp, "Mã giảm giá đã tồn tại", 409);
                    return;
                }
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if (couponDAO.hasReferences(id)) {
                ApiResponse.error(
                    resp,
                    "Không thể xóa mã đã có lịch sử nhận hoặc sử dụng",
                    409
                );
                return;
            }
    ...
    private void apply(
        ...
        if (creating || body.containsKey("code")) coupon.setCode(
            code(body.get("code"))
        );
        if (creating || body.containsKey("type")) coupon.setType(
            type(body.get("type"))
        );
        if (creating || body.containsKey("value")) coupon.setValue(
            amount(body.get("value"), "Giá trị")
        );
        if (creating || body.containsKey("minOrder")) coupon.setMinOrder(
            amountOrZero(body.get("minOrder"), "Đơn tối thiểu")
        );
        if (creating || body.containsKey("maxDiscount")) coupon.setMaxDiscount(
            optionalAmount(body.get("maxDiscount"), "Giảm tối đa")
        );
        if (creating || body.containsKey("maxUses")) coupon.setMaxUses(
            nonNegativeInt(body.get("maxUses"), "Số lần dùng tối đa")
        );
        if (creating || body.containsKey("expiresAt")) coupon.setExpiresAt(
            expiry(body.get("expiresAt"))
        );
        if (body.containsKey("isActive")) coupon.setIsActive(
            bool(body.get("isActive"), "Trạng thái kích hoạt")
        );
        if (creating || body.containsKey("isPublic")) coupon.setIsPublic(
            body.containsKey("isPublic")
                ? bool(body.get("isPublic"), "Trạng thái công khai")
                : true
        );
        if (
            creating ||
            ...
            body.containsKey("minOrder") ||
            body.containsKey("maxDiscount") ||
            body.containsKey("maxUses") ||
            body.containsKey("expiresAt")
        ) validate(coupon);
    }

    private String code(Object value) {
        if (
            !(value instanceof String code) ||
            !code.trim().matches("[A-Za-z0-9_-]{3,50}")
        ) throw new IllegalArgumentException(
            "Mã chỉ gồm chữ, số, dấu gạch nối hoặc gạch dưới (3-50 ký tự)"
        );
    ...
    private String type(Object value) {
        if (
            !(value instanceof String type) || !TYPES.contains(type)
    ...
    private LocalDateTime expiry(Object value) {
        if (
            value == null || (value instanceof String text && text.isBlank())
        ) return null;
    ...
    private void validate(Coupon coupon) {
        ...
        if (
            "FIXED".equals(coupon.getType()) && coupon.getValue().signum() <= 0
