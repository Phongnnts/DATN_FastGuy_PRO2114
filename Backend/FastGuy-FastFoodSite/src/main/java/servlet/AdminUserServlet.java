...
public class AdminUserServlet extends HttpServlet {
    ...
    static String mutationConflict(
        int targetUserId,
        int actorUserId,
        String role,
        String status,
        ...
        if (targetUserId == actorUserId) {
            if (
                "DELETE".equals(mutation)
            ) return "Không thể xóa tài khoản của chính bạn";
            if (
                "DISABLE".equals(mutation)
            ) return "Không thể vô hiệu hóa tài khoản của chính bạn";
            if (
                "DEMOTE".equals(mutation)
            ) return "Không thể hạ quyền quản trị của chính bạn";
        }
        if (
            "ADMIN".equals(role) &&
            "ACTIVE".equals(status) &&
            Set.of("DELETE", "DISABLE", "DEMOTE").contains(mutation) &&
            activeAdmins <= 1
        ) {
    ...
    private Map<String, Object> toMap(User user) {
        ...
        result.put("roleName", user.getRole() != null ? user.getRole() : "");
    ...
    private List<entity.Orders> ordersFor(User user) {
        return switch (orderRelationship(user.getRole())) {
            ...
            case "SHIPPER" -> ordersDAO.findByShipperIdAndStatus(
                user.getUserId(),
                "DELIVERED"
            );
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (path == null || path.equals("/")) {
            ...
            List<Map<String, Object>> list = userDAO
                .findAll()
                .stream()
                .map(this::toMap)
            ...
            if (roleFilter != null && !roleFilter.isBlank()) {
                ...
                list = list
                    .stream()
                    .filter(m -> rf.equals(m.get("roleName")))
                    .collect(Collectors.toList());
            }
        ...
        try {
            ...
            if (parts.length >= 3 && "orders".equals(parts[2])) {
                List<Map<String, Object>> orders = ordersFor(user)
                    .stream()
                    .map(o -> {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            User user = new User();
            apply(body, user, true);
            if (userDAO.findByEmail(user.getEmail()) != null) {
                ApiResponse.error(resp, "Email already exists", 409);
                return;
            }
            if (
                user.getPhone() != null &&
                ...
                ApiResponse.error(resp, "Số điện thoại đã tồn tại", 409);
                return;
            }
            ...
            userDAO.save(user);
            resp.setStatus(201);
            ApiResponse.ok(resp, toMap(user), "Created");
        } catch (IllegalArgumentException e) {
            ApiResponse.error(resp, e.getMessage(), 400);
        } catch (PersistenceException e) {
            ApiResponse.error(resp, "Không thể lưu người dùng", 409);
        }
    }
    ...
    protected synchronized void doPut(
        ...
        if (parts.length >= 3 && "status".equals(parts[2])) {
            try {
                ...
                String status =
                    body != null ? (String) body.get("status") : null;
                if (
                    status == null ||
                ...
                String mutation = "INACTIVE".equals(status)
                    ? "DISABLE"
                    : "ENABLE";
                String conflict = mutationConflict(
                    userId,
                    actorUserId(req),
                    user.getRole(),
                    user.getStatus(),
                    mutation,
                    userDAO.countActiveAdmins()
                );
                if (conflict != null) {
                    ApiResponse.error(resp, conflict, 409);
                    return;
                }
        ...
        try {
            User user = userDAO.findById(id(req));
            if (user == null) {
                ApiResponse.error(resp, "Not found", 404);
                return;
            }
            ...
            if (
                body.containsKey("roleName") &&
                !"ADMIN".equals(body.get("roleName")) &&
            ...
            apply(body, user, false);
            if (!oldEmail.equals(user.getEmail())) {
                User existing = userDAO.findByEmail(user.getEmail());
                if (
                    existing != null && existing.getUserId() != user.getUserId()
                ) {
                    ApiResponse.error(resp, "Email already exists", 409);
                    return;
                }
            ...
            userDAO.save(user);
            ApiResponse.ok(resp, toMap(user), "Updated");
        } catch (NumberFormatException e) {
            ApiResponse.error(resp, "Invalid ID", 400);
        } catch (IllegalArgumentException e) {
            ApiResponse.error(resp, e.getMessage(), 400);
    ...
    protected synchronized void doDelete(
        ...
        try {
            ...
            ApiResponse.error(
                resp,
                "Không thể xóa người dùng đang được sử dụng",
                409
            );
        }
    }
    ...
    private void apply(Map<String, Object> body, User user, boolean creating) {
        if (creating || body.containsKey("fullName")) {
            Object raw = body.get("fullName");
            if (
        ...
        if (creating || body.containsKey("email")) {
            ...
            if (
                !(raw instanceof String value) ||
                !value.trim().matches("^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$")
            ) throw new IllegalArgumentException("Email không hợp lệ");
            user.setEmail(value.trim().toLowerCase());
        }
        if (body.containsKey("phone")) {
            Object raw = body.get("phone");
            if (
                raw != null && !(raw instanceof String)
            ) throw new IllegalArgumentException("Số điện thoại không hợp lệ");
            user.setPhone(raw == null ? null : ((String) raw).trim());
        }
        if (body.containsKey("avatarUrl")) {
            Object raw = body.get("avatarUrl");
            user.setAvatarUrl(UserAvatarPolicy.normalize(raw));
        }
        if (creating || body.containsKey("roleName")) {
            Object raw = body.getOrDefault("roleName", "USER");
            if (
                !(raw instanceof String name) || !ROLES.contains(name)
            ) throw new IllegalArgumentException("Vai trò không hợp lệ");
            user.setRole(name);
        }
        if (creating || body.containsKey("password")) {
            Object raw = body.get("password");
            if (
                !(raw instanceof String password) ||
                password.length() < 6 ||
                password.length() > 72
            ) throw new IllegalArgumentException(
