...
public final class OrderExpiryPolicy {
    ...
    private static final Set<String> STORE_HELD = Set.of(
        "PENDING",
        "CONFIRMED",
        "PREPARING",
        "READY"
    );
    private static final Map<String, Long> MINUTES = Map.of(
        "PENDING",
        10L,
        "CONFIRMED",
        15L,
    ...
    public static Metadata metadata(Orders order, LocalDateTime now) {
        ...
        if (
            entered == null ||
            now == null ||
            !STORE_HELD.contains(order.getOrderStatus())
        ) return new Metadata(entered, null, null, null);
        LocalDateTime expires = entered.plusMinutes(timeoutMinutes(order));
        return new Metadata(
            entered,
            expires,
            Math.max(0, Duration.between(now, expires).getSeconds()),
            "AUTO_CANCEL"
        );
    }
    ...
    private static long timeoutMinutes(Orders order) {
        return "PENDING".equals(order.getOrderStatus()) &&
            ...
            ? 15
            : MINUTES.get(order.getOrderStatus());
    }
