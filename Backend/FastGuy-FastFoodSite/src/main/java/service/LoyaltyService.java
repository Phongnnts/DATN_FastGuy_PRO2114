...
public class LoyaltyService {
    ...
    public int awardForDelivery(EntityManager em, Orders order) {
        if (
            ...
            hasType(em, order.getOrderId(), "EARN")
        ) return 0;
    ...
    static int pointsForAmount(BigDecimal amount) {
        return amount == null
            ? 0
            : amount.divideToIntegralValue(BigDecimal.valueOf(1000)).intValue();
    }
    ...
    public Map<String, Object> getForUser(int userId) {
        ...
        try {
            ...
            List<LoyaltyTransaction> transactions = em
                .createQuery(
                    "SELECT lt FROM LoyaltyTransaction lt WHERE lt.user.userId = :userId ORDER BY lt.createdAt DESC",
    ...
    public void reverseForRefund(EntityManager em, Orders order) {
        if (
            order.getUser() == null ||
            hasType(em, order.getOrderId(), "REVERSE")
        ) return;
        List<LoyaltyTransaction> earns = em
            .createQuery(
                "SELECT lt FROM LoyaltyTransaction lt WHERE lt.order.orderId = :orderId AND lt.transactionType = 'EARN'",
                LoyaltyTransaction.class
            )
    ...
    private boolean hasType(EntityManager em, int orderId, String type) {
        return !em
            .createQuery(
                "SELECT lt FROM LoyaltyTransaction lt WHERE lt.order.orderId = :orderId AND lt.transactionType = :type",
                LoyaltyTransaction.class
            )
