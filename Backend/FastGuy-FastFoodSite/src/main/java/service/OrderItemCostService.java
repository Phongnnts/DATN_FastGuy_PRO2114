...
public class OrderItemCostService {
    ...
    public static BigDecimal recipeUnitCost(BigDecimal yield, List<CostLine> lines) {
        if (yield == null || yield.signum() <= 0 || lines == null || lines.isEmpty()) return null;
        BigDecimal total = BigDecimal.ZERO;
        for (CostLine line : lines) {
            if (line == null || line.quantity() == null || line.quantity().signum() <= 0
                    || line.unitCost() == null || line.unitCost().signum() <= 0) return null;
    ...
    public void snapshot(EntityManager em, Orders order) {
        List<OrderItem> orderItems = em.createQuery("SELECT oi FROM OrderItem oi JOIN FETCH oi.variant WHERE oi.order.orderId=:orderId ORDER BY oi.orderItemId", OrderItem.class)
        ...
        for (OrderItem orderItem : orderItems) {
            ...
            orderItem.setTotalCostSnapshot(moneyCost.multiply(BigDecimal.valueOf(orderItem.getQuantity())).setScale(2, RoundingMode.HALF_UP));
        }
    }
    ...
    private BigDecimal variantCost(EntityManager em, ProductVariant variant) {
        ...
        if ("FINISHED_GOOD".equals(variant.getInventoryMode())) {
            List<VariantInventoryItem> mappings = em.createQuery("SELECT m FROM VariantInventoryItem m JOIN FETCH m.inventoryItem WHERE m.variant.variantId=:variantId", VariantInventoryItem.class)
                    .setParameter("variantId", variant.getVariantId()).getResultList();
            if (mappings.size() != 1) return null;
            BigDecimal cost = mappings.get(0).getInventoryItem().getAverageUnitCost();
        ...
        return recipeUnitCost(recipe.getYieldQuantity(), recipe.getItems().stream()
                .map(line -> new CostLine(line.getQuantity(), line.getInventoryItem().getAverageUnitCost())).toList());
    }
}
