...
public class MailService {
    ...
    public void sendPasswordReset(String recipient, String token) {
        if (!AppConfig.isSmtpConfigured()) throw new IllegalStateException(
            "Email đặt lại mật khẩu chưa được cấu hình"
        );
        ...
        try {
            ...
            String resetUrl =
                AppConfig.getAppWebUrl() + "/reset-password?token=" + token;
            message.setText(
                "Bạn đã yêu cầu đặt lại mật khẩu FastGuy. Liên kết có hiệu lực trong 15 phút:\n" +
                    resetUrl +
