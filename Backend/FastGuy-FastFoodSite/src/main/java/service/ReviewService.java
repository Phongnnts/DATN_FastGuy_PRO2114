...
public class ReviewService {
    ...
    public Map<String, Object> getByOrderId(int userId, int orderId) {
        List<Map<String, Object>> reviews = reviewDAO.findAllByUserOrder(userId, orderId).stream()
                .sorted(Comparator.comparingInt(review -> review.getProduct().getProductId()))
    ...
    public Map<String, Object> getByProductId(int productId, int page, int size) {
        if (page < 1 || size < 1 || size > 50) throw new IllegalArgumentException("Trang đánh giá không hợp lệ");
        ReviewDAO.ProductReviewAggregate aggregate = reviewDAO.aggregateByProductId(productId);
        long count = aggregate == null ? 0 : aggregate.reviewCount();
        List<Map<String, Object>> items = reviewDAO.findPublicByProductId(productId, page, size).stream()
                .sorted(Comparator.comparing(ReviewDAO.PublicReview::createdAt).reversed()
                        .thenComparing(ReviewDAO.PublicReview::reviewId, Comparator.reverseOrder()))
        ...
        result.put("averageRating", aggregate == null || aggregate.averageRating() == null
                ? new BigDecimal("0.0") : BigDecimal.valueOf(aggregate.averageRating()).setScale(1, RoundingMode.HALF_UP));
    ...
    public Map<String, Object> create(int userId, int orderId, int productId, int rating, String comment,
        ...
        return toCustomerMap(reviewDAO.save(userId, orderId, productId, rating, comment, homepageConsent));
    }
    ...
    private Map<String, Object> toAdminMap(Review review) {
        Map<String, Object> data = new LinkedHashMap<>();
        data.put("reviewId", review.getReviewId());
        data.put("productId", review.getProduct() == null ? null : review.getProduct().getProductId());
        data.put("rating", review.getRating());
        data.put("comment", review.getComment());
        data.put("createdAt", review.getCreatedAt());
