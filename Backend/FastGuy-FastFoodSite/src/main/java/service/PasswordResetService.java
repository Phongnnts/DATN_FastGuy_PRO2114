    ...
    public void reset(String token, String password) {
        if (!AuthService.isStrongPassword(password)) {
            throw new IllegalArgumentException(
                "Mật khẩu mới phải từ 8 ký tự, có ít nhất 1 chữ và 1 số"
            );
        }
        PasswordResetToken reset = tokenDAO.findUsableByHash(hashToken(token));
        if (reset == null) throw new IllegalArgumentException(
            "Liên kết đặt lại mật khẩu không hợp lệ hoặc đã hết hạn"
        );
        tokenDAO.consumeAndUpdatePassword(
            reset.getResetTokenId(),
