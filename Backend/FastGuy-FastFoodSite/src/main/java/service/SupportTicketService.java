...
public class SupportTicketService {
    ...
    public Map<String, Object> create(int userId, Integer orderId, String subject, String category, String description) {
        ...
        try {
            ...
            try {
                notificationService.notifyRole("STAFF", "Yêu cầu hỗ trợ mới", "Yêu cầu #" + ticket.getTicketId() + ": " + ticket.getSubject(), "SUPPORT_TICKET", "/staff/support");
    ...
    public Map<String, Object> update(int ticketId, int staffId, String status, String resolution) {
        ...
        try {
            ...
            if (changed) {
                try {
                    notificationService.notifyUser(ticket.getUser().getUserId(), "Cập nhật hỗ trợ", "Yêu cầu #" + ticketId + " đã chuyển sang " + statusLabel(status), "SUPPORT_TICKET", "/account/support");
                } catch (RuntimeException ignored) {
                }
            }
            return result;
    ...
    private String statusLabel(String status) {
        return Map.of("OPEN", "Mới", "PROCESSING", "Đang xử lý", "RESOLVED", "Đã giải quyết").get(status);
    }
}
