...
public class CouponService {
    ...
    public Map<String, Object> verify(
        ...
        if (totalAmount.compareTo(minOrder) < 0) {
            ...
            result.put(
                "message",
                "Đơn hàng tối thiểu " + minOrder.toPlainString() + "đ"
            );
            return result;
        }
        ...
        if (userId == null || userId <= 0) {
            ...
            result.put(
                "message",
                "Vui lòng đăng nhập và nhận mã trước khi sử dụng"
            );
            return result;
        }
    ...
    public String claim(int couponId, int userId) {
        ...
        try {
            ...
            if (
                !Boolean.TRUE.equals(coupon.getIsPublic()) ||
            ...
            if (
                ...
                em.getTransaction().rollback();
                return "Mã đã hết lượt";
            }

            Long count = em
                .createQuery(
                    "SELECT COUNT(cr) FROM CouponRedemption cr WHERE cr.couponId = :cid AND cr.userId = :uid",
                    Long.class
                )
    ...
    public List<Map<String, Object>> getPublicCoupons(Integer userId) {
        ...
        if (userId != null) {
            claimedIds = redemptionDAO
                .findByUserId(userId)
                .stream()
                .map(cr -> cr.getCouponId())
        ...
        return list
            ...
            .map(c -> {
                ...
                m.put(
                    "isClaimed",
                    finalClaimed != null &&
                        finalClaimed.contains(c.getCouponId())
                );
                return m;
            })
        ...
        return redemptionDAO
            ...
            .map(cr -> {
                ...
                if (
                    ...
                    (c.getMaxUses() > 0 && c.getUsedCount() >= c.getMaxUses())
                ) return null;
                Map<String, Object> m = new HashMap<>();
                ...
                m.put(
                    "minOrder",
                    c != null ? c.getMinOrder() : BigDecimal.ZERO
                );
                m.put(
                    "maxDiscount",
                    c != null ? c.getMaxDiscount() : BigDecimal.ZERO
                );
                m.put("description", c != null ? getDescription(c) : "");
                m.put(
                    "claimedAt",
                    cr.getClaimedAt() != null
                        ? cr.getClaimedAt().toString()
                        : null
                );
                return m;
            })
    ...
    private String getDescription(Coupon c) {
        switch (c.getType()) {
            case "PERCENT":
                return "Giảm " + c.getValue().intValue() + "%";
            case "FIXED":
                return "Giảm " + c.getValue().intValue() + "đ";
            case "FREE_SHIPPING":
