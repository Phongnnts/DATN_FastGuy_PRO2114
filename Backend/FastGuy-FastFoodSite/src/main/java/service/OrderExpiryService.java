...
public class OrderExpiryService {
    ...
    public OrderTransitionService.CancellationResult cancelExpired(
        int orderId,
        LocalDateTime now
    ) {
    ...
    public OrderTransitionService.CancellationResult cancelAtCutoff(
        int orderId,
        LocalDateTime now
    ) {
    ...
    public void cancelCutoffCandidates(LocalDateTime now) {
        ...
        attempt(ordersDAO.findCutoffCandidates(), order ->
            cancelAtCutoff(order.getOrderId(), now)
        );
    }

    public void cancelExpiredCandidates(LocalDateTime now) {
        attempt(findCandidates(now), order ->
            cancelExpired(order.getOrderId(), now)
        );
    }

    private void attempt(
        List<Orders> candidates,
        java.util.function.Consumer<Orders> cancellation
    ) {
        for (Orders order : candidates) {
            try {
                cancellation.accept(order);
