...
public class RefundProofStorage {
    ...
    public UploadedProof uploadPrivate(byte[] content, String contentType) {
        return uploadPrivate("proof", content, contentType);
    }

    public UploadedProof uploadPrivate(
        String identity,
        byte[] content,
        String contentType
    ) {
        ...
        try {
            ...
            ) throw new IllegalStateException("Refund proof upload failed");
            JsonNode json = JsonUtil.getMapper().readTree(response.body());
            String stored = json.path("public_id").asText();
            if (stored.isBlank()) throw new IllegalStateException(
                "Refund proof upload failed"
            );
            ...
            throw new IllegalStateException(
                "Refund proof upload interrupted",
                e
            );
        }
    }
        ...
        for (int attempt = 0; attempt < 2; attempt++) {
            try {
                ...
                failure = new IllegalStateException(
                    "Refund proof cleanup failed"
                );
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new IllegalStateException(
                    "Refund proof cleanup interrupted",
                    e
                );
            } catch (IOException e) {
                failure = new IllegalStateException(
    ...
    public SignedProofUrl signedViewUrl(String publicId, Duration ttl) {
        return signedViewUrl(publicId, "image/jpeg", ttl);
    }

    public SignedProofUrl signedViewUrl(
        String publicId,
        String contentType,
        Duration ttl
    ) {
        ...
        ) throw new IllegalArgumentException("Refund proof is unavailable");
    ...
    public static String publicId(String identity, byte[] content) {
        return "fastguy/refunds/" + identity + "-" + sha256(content);
    }
    ...
    public static void validate(byte[] content, String contentType) {
        ...
        ) throw new IllegalArgumentException("Invalid refund proof");
        boolean valid = switch (contentType) {
            ...
            case "image/webp" -> content.length >= 12 &&
                new String(content, 0, 4, StandardCharsets.US_ASCII).equals(
                    "RIFF"
                ) &&
                new String(content, 8, 4, StandardCharsets.US_ASCII).equals(
                    "WEBP"
                );
            default -> false;
        };
        if (!valid) throw new IllegalArgumentException(
            "Invalid refund proof content"
        );
    }
    ...
    private static byte[] multipart(
        ...
        String fields =
            part(boundary, "public_id", publicId) +
            part(boundary, "timestamp", String.valueOf(timestamp)) +
            part(boundary, "type", "authenticated") +
    ...
    private static String sha1(String value) {
        try {
            ...
            throw new IllegalStateException("Proof signing unavailable", e);
        }
    }
    ...
    private static String sha256(byte[] value) {
        try {
            ...
            throw new IllegalStateException("Proof identity unavailable", e);
        }
    }
