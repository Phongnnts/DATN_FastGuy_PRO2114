...
public class InventoryReservationService {
    ...
    public InventoryReservationService() {
        this(new InventoryAvailabilityService(), new InventoryItemDAO());
    }

    InventoryReservationService(
        InventoryAvailabilityService availability,
        InventoryItemDAO items
    ) {
    ...
    public static boolean canTransition(String from, String to) {
        return (
            ("RESERVED".equals(from) &&
                Set.of("CONSUMED", "RELEASED").contains(to)) ||
    ...
    public void reserve(
        ...
        Map<Integer, BigDecimal> demand = availability.aggregateDemand(
            em,
            variantQuantities
        );
        ...
        List<InventoryItem> locked = itemIds
            .stream()
            .map(id -> items.lock(em, id))
            .toList();
        if (
            locked.stream().anyMatch(i -> i == null || !i.isActive())
        ) throw new IllegalStateException("Inventory item unavailable");
        for (InventoryItem item : locked)
            if (
                demand
                    .get(item.getInventoryItemId())
                    .compareTo(item.availableQuantity()) > 0
    ...
    public boolean consume(EntityManager em, Orders order) {
        ...
        if (
            reservation == null ||
            !canTransition(reservation.getStatus(), "CONSUMED")
        ) return false;
        for (InventoryReservationItem line : reservation
            .getItems()
            .stream()
            .sorted((a, b) ->
                Integer.compare(
                    a.getInventoryItem().getInventoryItemId(),
                    b.getInventoryItem().getInventoryItemId()
                )
            )
            .toList()) {
            InventoryItem item = items.lock(
                em,
            ...
            item.setOnHandQuantity(
                item.getOnHandQuantity().subtract(line.getQuantity())
            );
            record(
                em,
                order,
    ...
    public boolean release(EntityManager em, Orders order) {
        ...
        if (
            reservation == null ||
            !canTransition(reservation.getStatus(), "RELEASED")
        ) return false;
        for (InventoryReservationItem line : reservation
            .getItems()
            .stream()
    ...
    public boolean transition(EntityManager em, Orders order, String toStatus) {
        return "CONSUMED".equals(toStatus)
            ? consume(em, order)
            : "RELEASED".equals(toStatus) && release(em, order);
    }
    ...
    public boolean cancel(EntityManager em, Orders order) {
        InventoryReservation reservation = findByOrder(em, order.getOrderId());
        if (reservation == null) return hasLegacyReservationEvidence(
            em,
            order.getOrderId()
        );
        if (
            reservation.getItems() == null || reservation.getItems().isEmpty()
        ) return false;
        if ("RESERVED".equals(reservation.getStatus())) return release(
            em,
            order
        );
    ...
    public boolean hasReservations(EntityManager em, int orderId) {
        return findByOrder(em, orderId) != null;
    }

    private boolean hasLegacyReservationEvidence(
        EntityManager em,
        int orderId
    ) {
        Number count = (Number) em
            .createNativeQuery(
                "SELECT COUNT_BIG(*) FROM InventoryReservationLegacyHistory WHERE order_id = :orderId"
            )
    ...
    private boolean equivalent(
        ...
        Map<Integer, BigDecimal> demand
    ) {
        if (
            !"RESERVED".equals(reservation.getStatus()) ||
            reservation.getItems() == null ||
            reservation.getItems().size() != demand.size()
        ) return false;
        for (InventoryReservationItem line : reservation.getItems()) {
            BigDecimal expected = demand.get(
                line.getInventoryItem().getInventoryItemId()
            );
            if (
                expected == null || expected.compareTo(line.getQuantity()) != 0
            ) return false;
        }
        return true;
    }
    ...
    private InventoryReservation findByOrder(EntityManager em, int orderId) {
        List<InventoryReservation> reservations = em
            .createQuery(
                "SELECT DISTINCT r FROM InventoryReservation r LEFT JOIN FETCH r.items ri LEFT JOIN FETCH ri.inventoryItem WHERE r.order.orderId = :orderId",
    ...
    private void record(
        ...
        if (Set.of("CONSUME", "WASTE").contains(type)) {
            ...
            transaction.setTotalCost(
                quantity.abs().multiply(cost).setScale(4, RoundingMode.HALF_UP)
            );
        }
        em.persist(transaction);
    }

    private BigDecimal scale(BigDecimal quantity) {
