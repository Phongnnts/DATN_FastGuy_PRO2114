...
test('inventory separates stock-health overview from auditable history', () => {
  assert.match(inventory, /class="command-rail inventory-command-rail"/);
  assert.match(inventory, /inventory-risk-command/);
  assert.match(inventory, /available-quantity/);
...
test('ledger remains bounded on desktop and becomes evidence cards on mobile', () => {
  assert.match(ledger, /ledger-evidence-workspace/);
