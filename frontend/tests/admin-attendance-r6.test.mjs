...
test('attendance and pay rates are separate accessible modes',()=>{for(const token of ['attendance-mode-tabs','attendance-summary-grid','attendance-toolbar','attendance-workspace','pay-rate-workspace','pay-rate-history'])assert.match(page,new RegExp(token));assert.match(page,/const attendanceView = ref\('attendance'\)/);assert.match(page,/role="tablist"/);assert.match(page,/Duyệt chấm công/);assert.match(page,/Mức tiền công/);assert.match(page,/aria-selected/);});
