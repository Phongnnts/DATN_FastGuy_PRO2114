...
<template>
  ...
  <div v-else-if="order"><router-link class="back-link" :to="backTo">← Quay lại</router-link><header class="detail-header"><div><h1>{{ order.orderCode }}</h1><time :datetime="order.createdAt">Tạo {{ formatDate(order.createdAt) }}</time></div><OrderStatusBadge :status="order.status" /></header>
    ...
    <section class="info-card"><h2>Thanh toán</h2><div class="row"><span>Phương thức</span><strong>{{ order.paymentMethod }}</strong></div><div class="row"><span>Trạng thái</span><strong>{{ order.paymentStatus }}</strong></div><div class="row"><span>Phí giao</span><span>{{ formatPrice(order.shippingFee) }}</span></div><div v-if="order.serviceFee > 0" class="row"><span>Phí dịch vụ</span><span>{{ formatPrice(order.serviceFee) }}</span></div><div v-if="order.discount > 0" class="row"><span>Giảm giá</span><span>-{{ formatPrice(order.discount) }}</span></div><div class="row total"><span>Tổng thu</span><strong>{{ formatPrice(order.total) }}</strong></div><div v-if="order.codCollectedAmount != null" class="row"><span>COD đã thu</span><strong>{{ formatPrice(order.codCollectedAmount) }}</strong></div></section>
    <section class="info-card"><h2>Mốc thời gian</h2><p v-if="waitingMinutes != null">Đã chờ {{ waitingMinutes }} phút</p><p v-if="order.assignedAt">Phân công: {{ formatDate(order.assignedAt) }}</p><p v-if="order.pickedUpAt">Lấy hàng: {{ formatDate(order.pickedUpAt) }}</p><p v-if="order.deliveredAt">Giao hàng: {{ formatDate(order.deliveredAt) }}</p><OrderTimeline :history="order.statusHistory" /></section>
