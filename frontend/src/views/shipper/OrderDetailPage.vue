...
import { shiftApi, shipperApi, storeApi } from '@/api';
import { useShipperStore } from '@/stores/shipper';
import { DELIVERY_FAILURE_REASON_LABEL } from '@/utils/constants';
import { formatPrice, formatDate } from '@/utils/format';
import { deliveryFailureLabel, failureFocusTarget, failureRestoreTarget, validateExactCod } from '@/utils/shipperOperations';
...
const previousFailureFocus = ref(null);
const failureFocusFallback = ref(null);
...
const isTerminal = computed(() => ['DELIVERED', 'CANCELLED', 'RETURNED_TO_STORE'].includes(order.value?.status));
const isReadOnlyFailure = computed(() => ['DELIVERY_FAILED', 'RETURNED_TO_STORE'].includes(order.value?.status));
const backTo = computed(() => isTerminal.value || isReadOnlyFailure.value ? '/shipper/history' : '/shipper/orders');
const allowedActions = computed(() => checkedIn.value && !isReadOnlyFailure.value ? order.value?.allowedActions || [] : []);
...
const canFail = computed(() => checkedIn.value && order.value?.status === 'PICKED_UP');
const canSubmitFailure = computed(() => Boolean(failureReason.value && failureNote.value.trim() && failureNote.value.trim().length <= 500 && !submitting.value));
...
async function openFailure() {
  ...
  previousFailureFocus.value = document.activeElement;
  failureError.value = '';
  failureOpen.value = true;
  document.addEventListener('focusin', handleFailureFocusIn);
  document.addEventListener('keydown', handleFailureKeydown);
  await nextTick();
  failureDialog.value?.focus();
}
function restoreFailureFocus() {
  failureRestoreTarget(previousFailureFocus.value, failureFocusFallback.value)?.focus();
  previousFailureFocus.value = null;
}
function removeFailureListeners() {
  document.removeEventListener('focusin', handleFailureFocusIn);
  document.removeEventListener('keydown', handleFailureKeydown);
}
async function closeFailure(force = false, restore = true) {
  if (submitting.value && !force) return;
  failureOpen.value = false;
  removeFailureListeners();
  await nextTick();
  if (restore) restoreFailureFocus();
}
function handleFailureFocusIn(event) {
  if (failureOpen.value && failureDialog.value && !failureDialog.value.contains(event.target)) failureDialog.value.focus();
}
function handleFailureKeydown(event) {
  if (event.key === 'Escape') { event.preventDefault(); closeFailure(); return; }
  if (event.key !== 'Tab' || !failureDialog.value) return;
  const focusable = [...failureDialog.value.querySelectorAll('button:not([disabled]), select:not([disabled]), textarea:not([disabled])')];
  if (!focusable.length) return;
  const targetIndex = failureFocusTarget({ activeIndex: focusable.indexOf(document.activeElement), lastIndex: focusable.length - 1, shiftKey: event.shiftKey });
  if (targetIndex != null) { event.preventDefault(); focusable[targetIndex].focus(); }
}
async function submitFailure() {
  if (!order.value || !canSubmitFailure.value) return;
  submitting.value = true;
  failureError.value = '';
  try {
    await shipperApi.failOrder(order.value.id, { expectedStatus: order.value.status, reasonCode: failureReason.value, note: failureNote.value.trim() });
    await closeFailure(true, false);
    failureReason.value = '';
    failureNote.value = '';
    await load();
    await nextTick();
    ...
    failureError.value = error?.status === 409 ? 'Đơn hàng đã thay đổi trạng thái. Biểu mẫu được giữ lại và dữ liệu mới nhất đã được tải.' : error.message || 'Không thể báo giao thất bại';
    toast.error(failureError.value);
...
onUnmounted(() => { stopped = true; loadGeneration += 1; removeFailureListeners(); store.invalidateDetailRequests(); clearInterval(clockTimer); });
</script>
...
<template>
  ...
  <div v-else-if="order" class="order-detail-grid"><router-link class="back-link" :to="backTo">← Quay lại</router-link><header class="detail-header order-command-header"><div><span class="detail-kicker">Lệnh giao hàng</span><h1>{{ order.orderCode }}</h1><time :datetime="order.createdAt">Tạo {{ formatDate(order.createdAt) }}</time></div><OrderStatusBadge :status="order.status" /></header>
    ...
    <section v-if="canDeliver && order.paymentMethod === 'COD'" class="info-card"><label for="cod-amount">Số tiền COD đã thu</label><input id="cod-amount" v-model="collectedAmount" class="form-input" type="number" min="0" step="0.01" :aria-invalid="!!actionError" :aria-describedby="actionError ? 'action-error' : undefined" /></section><p v-if="actionError" id="action-error" class="error" role="alert">{{ actionError }}</p><p v-if="order.allowedActions.length && !checkedIn" class="state">Check-in ca làm để thao tác đơn</p><div ref="failureFocusFallback" class="sticky-actions" data-surface="sticky-command-bar" tabindex="-1"><button v-if="canPickup" class="btn btn-primary" :disabled="submitting" @click="requestAction('pickup')">{{ submitting ? 'Đang xử lý...' : 'Đã lấy hàng' }}</button><button v-if="canDeliver" class="btn btn-success" :disabled="submitting" @click="requestAction('deliver')">{{ submitting ? 'Đang xử lý...' : 'Đã giao thành công' }}</button><button v-if="canFail" class="btn btn-danger" :disabled="submitting" @click="openFailure">Báo giao thất bại</button><button class="btn btn-outline" @click="openIssue">Báo sự cố</button></div>
    <section v-if="isReadOnlyFailure" class="info-card" aria-live="polite"><h2>Thông tin giao thất bại</h2><p>Lần giao {{ order.deliveryAttemptCount }} / {{ order.deliveryAttemptLimit }}</p><p v-if="order.deliveryAttemptCount >= order.deliveryAttemptLimit" class="error">Đã đạt giới hạn giao. Đơn chờ cửa hàng xử lý.</p><p><strong>Lý do:</strong> {{ deliveryFailureLabel(order.deliveryFailureCode) }}</p><p v-if="order.failureNote"><strong>Ghi chú:</strong> {{ order.failureNote }}</p></section>
    ...
    <details class="info-card"><summary>Chi tiết phí</summary><div class="row"><span>Phí giao</span><span>{{ formatPrice(order.shippingFee) }}</span></div><div v-if="order.discount > 0" class="row"><span>Giảm giá</span><span>-{{ formatPrice(order.discount) }}</span></div></details>
    ...
    <div v-if="failureOpen" class="modal-overlay" @click.self="closeFailure()"><section ref="failureDialog" class="modal" role="dialog" aria-modal="true" aria-labelledby="failure-title" tabindex="-1"><div class="modal-header"><h2 id="failure-title" class="modal-title">Báo giao thất bại</h2><button type="button" class="modal-close" aria-label="Đóng" :disabled="submitting" @click="closeFailure"><i class="bi bi-x-lg" aria-hidden="true"></i></button></div><form class="modal-body failure-form" @submit.prevent="submitFailure"><p aria-live="polite">Lần giao {{ order.deliveryAttemptCount + 1 }} / {{ order.deliveryAttemptLimit }}</p><p v-if="order.deliveryAttemptCount + 1 >= order.deliveryAttemptLimit" class="warning">Đây là lần giao cuối trước khi cửa hàng phải xử lý.</p><label for="failure-reason">Lý do</label><select id="failure-reason" v-model="failureReason" class="form-select" required><option value="">Chọn lý do</option><option v-for="(label, code) in DELIVERY_FAILURE_REASON_LABEL" :key="code" :value="code">{{ label }}</option></select><label for="failure-note">Ghi chú</label><textarea id="failure-note" v-model="failureNote" class="form-input" maxlength="500" required :aria-invalid="Boolean(failureError)" aria-describedby="failure-help failure-error"></textarea><small id="failure-help">{{ failureNote.length }}/500 ký tự</small><p id="failure-error" class="error" aria-live="polite">{{ failureError }}</p><div class="modal-footer"><button type="button" class="btn btn-ghost" :disabled="submitting" @click="closeFailure">Hủy</button><button type="submit" class="btn btn-danger" :disabled="!canSubmitFailure">{{ submitting ? 'Đang gửi...' : 'Xác nhận thất bại' }}</button></div></form></section></div>
