...
async function retry() {
  ...
  inFlight = true;
  await Promise.allSettled([store.fetchDashboard(), store.fetchActiveOrders()]);
  if (!stopped) inFlight = false;
}

onMounted(retry);
onUnmounted(() => { stopped = true; store.invalidateListRequests(); });
</script>
...
<template>
  <div>
    ...
    <div v-else-if="!store.listLoading && !store.listError" class="state">Chưa có đơn đang hoạt động</div>
  </div>
</template>
...
.shipper-stats { display:grid; grid-template-columns:repeat(3,1fr); gap:8px; margin:12px 0; }.mini-stat { background:#fff; border:1px solid var(--border-light); border-radius:var(--radius); padding:12px 6px; text-align:center; }.mini-stat strong,.mini-stat span { display:block; }.mini-stat strong { color:var(--primary); font-size:24px; }.mini-stat span { color:var(--text-mid); font-size:11px; }
