...
async function load() { loading.value = true; loadError.value = ''; try { await staffStore.fetchDashboard(); } catch (e) { toast.error(e.message || 'Không thể tải tổng quan'); } finally { loading.value = false; } }
function handleVisibility() { if (document.visibilityState === 'visible') load(); }
onMounted(async () => { await load(); document.addEventListener('visibilitychange', handleVisibility); refreshTimer = setInterval(() => { if (document.visibilityState === 'visible') load(); }, 25000); });
watch(() => staffStore.dashboard, buildChart);
onUnmounted(() => { document.removeEventListener('visibilitychange', handleVisibility); if (refreshTimer) clearInterval(refreshTimer); destroyChart(); });
</script>
