...
async function handleTabKeydown(event, index) {
  ...
  await switchFilter(tabs[nextIndex].filter);
  await nextTick();
...
<template>
  <div>
    ...
    <div class="dispatch-tabs" role="tablist" aria-label="Bộ lọc điều phối">
      <button v-for="(tab, index) in tabs" :id="`dispatch-tab-${tab.filter}`" :key="tab.filter" :ref="(element) => { if (element) tabElements[index] = element; }" role="tab" class="dispatch-tab" :class="{ active: activeFilter === tab.filter }" :aria-selected="activeFilter === tab.filter" :aria-controls="`dispatch-panel-${tab.filter}`" :tabindex="activeFilter === tab.filter ? 0 : -1" @click="switchFilter(tab.filter)" @keydown="handleTabKeydown($event, index)"><span>{{ tab.label }}</span><span class="count-badge">{{ dispatchCounts[tab.countKey] }}</span></button>
    </div>
    ...
    <div :id="`dispatch-panel-${activeFilter}`" role="tabpanel" :aria-labelledby="`dispatch-tab-${activeFilter}`" tabindex="0">
...
<style scoped>
.page-header p{margin:4px 0 0;color:var(--text-mid);font-size:14px}.dispatch-tabs{display:flex;gap:8px;margin-bottom:12px;overflow-x:auto}.dispatch-tab{display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:44px;padding:8px 16px;border:1px solid var(--border-light);border-radius:var(--radius-sm);background:var(--bg-white);color:var(--text-mid);font-weight:700;cursor:pointer}.dispatch-tab.active{border-color:var(--primary);color:var(--primary)}.dispatch-tab:focus-visible{outline:3px solid var(--primary);outline-offset:2px}.count-badge,.status-badge{display:inline-block;padding:2px 7px;border-radius:999px;background:var(--bg-light);color:var(--text-dark);font-size:12px;font-weight:750}.status-badge{margin-top:5px}.lane-state{display:flex;flex-wrap:wrap;gap:12px;margin-bottom:12px;color:var(--text-mid)}.error{color:var(--red-active)}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.order-link{display:block;color:var(--text-dark);font-weight:750}.table td strong,.table td a,.table td small{display:block}.table td small{margin-top:4px;color:var(--text-mid)}.form-select{min-width:270px;min-height:44px}.target{display:inline-flex;align-items:center;justify-content:center;min-height:44px}@media(max-width:768px){.table thead{display:none}.table tbody tr{display:block;margin-bottom:8px;padding:12px;border:1px solid var(--border-light);border-radius:var(--radius-sm)}.table tbody td{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:6px 0;border:0;text-align:right}.table tbody td::before{content:attr(data-label);color:var(--text-mid);font-weight:650}.form-select{min-width:0;max-width:70%}}
</style>
