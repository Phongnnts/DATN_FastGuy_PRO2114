...
function remaining(order) { return createServerCountdown(order, order.receivedAt).remaining(now.value); }
...
async function assign(order) {
  ...
  try {
    ...
    if (!acceptsDispatchRequest({ requestGeneration, latestGeneration: generation, stopped })) return;
    delete selections.value[order.id];
    if (error.status === 409 || error.status === 410) selections.value = { ...selections.value };
...
onMounted(async () => {
  ...
  countdownTimer = setInterval(() => {
    now.value = Date.now();
    const expired = readyOrders.value.find((order) => order.remainingSeconds != null && remaining(order) === 0 && !refreshedAtZero.has(order.id));
    if (expired) { refreshedAtZero.add(expired.id); void loadOrders(); }
  }, 1000);
});
onBeforeUnmount(() => {
  stopped = true;
...
<template>
  <div>
    <div class="page-header"><div><h1>Điều phối giao hàng</h1><p>Ghép đơn sẵn sàng với shipper đang khả dụng.</p><p v-if="orderCutoffTime" class="cutoff-banner">Nhận đơn đến {{ orderCutoffTime }}.</p></div><button class="btn btn-sm btn-outline refresh-control" :disabled="ordersLoading || shippersLoading" @click="load">Làm mới</button></div>
