...
onMounted(async () => { activeTab.value = normalizedTab(route.query.tab); await refresh(); document.addEventListener('visibilitychange', handleVisibility); refreshTimer = setInterval(() => { if (document.visibilityState === 'visible') refresh(); }, 25000); });
onUnmounted(() => { document.removeEventListener('visibilitychange', handleVisibility); if (refreshTimer) clearInterval(refreshTimer); });
</script>
