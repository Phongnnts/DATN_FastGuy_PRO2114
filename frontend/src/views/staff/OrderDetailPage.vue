...
async function updateStatus(status, reason = null) {
  ...
  saving.value = true;
  try {
    await staffStore.updateOrderStatus(order.value.id, status, reason);
    if (!acceptsRequest(requestGeneration)) return;
    saving.value = false;
    await load({ silent: true });
  } catch (error) {
    if (acceptsRequest(requestGeneration)) toast.error(error.message || 'Không thể cập nhật trạng thái');
  } finally {
...
async function assignShipper() {
  ...
  try {
    ...
    if (acceptsRequest(requestGeneration)) toast.error(error.message || 'Không thể gán shipper');
  } finally {
    if (acceptsRequest(requestGeneration)) saving.value = false;
  }
}
...
<template>
  ...
  <div v-else-if="order">
    ...
    <div class="grid-2"><section class="card"><h3>Thông tin đơn hàng</h3><div class="info-row"><span>Khách hàng</span><strong>{{ order.customerName }}</strong></div><div class="info-row"><span>Điện thoại</span><strong><a v-if="order.customerPhone" :href="`tel:${order.customerPhone}`">{{ order.customerPhone }}</a><span v-else>Không có</span></strong></div><div class="info-row"><span>Địa chỉ</span><strong>{{ order.shippingAddress }}</strong></div><div class="info-row"><span>Phương thức</span><strong>{{ order.paymentMethod === 'BANK_TRANSFER' ? 'PayOS' : 'COD' }}</strong></div><div class="info-row"><span>Thanh toán</span><strong :class="{ paid: order.paymentStatus === 'PAID' }">{{ order.paymentStatus === 'PAID' ? 'Đã thanh toán' : 'Chưa thanh toán' }}</strong></div><div class="info-row"><span>Ghi chú giao hàng</span><strong>{{ order.note || 'Không có' }}</strong></div><div class="info-row"><span>Hoàn tiền</span><strong>{{ order.refundStatus || 'Không có' }}{{ order.refundNote ? ` · ${order.refundNote}` : '' }}</strong></div><div v-if="order.failureReason" class="info-row"><span>Lý do hủy</span><strong class="danger">{{ order.failureReason }}</strong></div></section>
    ...
    <section v-if="order.internalNotes.length" class="card mt-3"><h3>Ghi chú nội bộ</h3><ul class="notes"><li v-for="(note, index) in order.internalNotes" :key="note.id || index">{{ note.content || note.note || note }}</li></ul></section>
