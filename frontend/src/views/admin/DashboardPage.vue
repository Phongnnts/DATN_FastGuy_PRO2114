...
<template>
  <div>
    ...
    <div v-if="loading" class="admin-state">
      <span class="spinner"></span> Đang tải tổng quan...
    </div>

    <div v-else-if="error" class="admin-state admin-error">
      <span>{{ error }}</span>
      <button class="btn btn-sm btn-outline" @click="load">Thử lại</button>
    </div>

    <template v-else>
      <div class="empty-state" v-if="!adminStore.dashboard">
        <i class="bi bi-bar-chart-line"></i>
        <h3>Chưa có dữ liệu</h3>
        <p>Không tìm thấy dữ liệu tổng quan hệ thống.</p>
        <button class="btn btn-sm btn-outline" @click="load">Tải lại</button>
      </div>
