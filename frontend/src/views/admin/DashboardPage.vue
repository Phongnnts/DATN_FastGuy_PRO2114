  ...
  <div class="dashboard">
    ...
    <template v-else>
      ...
      <aside class="cash-risk-rail" aria-label="Dòng tiền và rủi ro">
        <router-link class="risk-item" data-cash-risk-section="refunds" :to="{ path: '/admin/refunds', query: { status: 'PENDING' } }"><span>Hoàn tiền đang chờ</span><strong>{{ available('refunds') ? Number(data.pendingRefundCount).toLocaleString('vi-VN') : 'Không khả dụng' }}</strong></router-link>
        <router-link class="risk-item" data-cash-risk-section="cod" :to="{ path: '/admin/cod-settlements', query: { status: 'SUBMITTED' } }"><span>COD đang chờ bàn giao</span><strong>{{ available('cod') ? Number(data.pendingCodCount).toLocaleString('vi-VN') : 'Không khả dụng' }}</strong></router-link>
        <router-link class="risk-item" data-cash-risk-section="staffing" :to="{ path: '/admin/shifts', query: { tab: 'monitoring' } }"><span>Ca làm thiếu nhân sự</span><strong>{{ available('staffing') ? Number(data.staffingGapCount).toLocaleString('vi-VN') : 'Không khả dụng' }}</strong></router-link>
      </aside>
