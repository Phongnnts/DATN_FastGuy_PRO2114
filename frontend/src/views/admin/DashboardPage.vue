...
const activeOrderTotal = computed(() => activeOrderSeries.value.reduce((sum, row) => sum + row.count, 0));
const revenueTotal = computed(() => revenueSeries.value.reduce((sum, row) => sum + Number(row.revenue), 0));
...
function buildCharts() {
  ...
  const surface = css('--admin-surface');
  if (available('financial') && revenueChartRef.value && revenueSeries.value.length) {
    const context = revenueChartRef.value.getContext('2d');
    const gradient = context.createLinearGradient(0, 0, 0, 280);
    gradient.addColorStop(0, `${brand}42`);
    gradient.addColorStop(1, `${brand}00`);
    revenueChart = new Chart(revenueChartRef.value, {
      type: 'line',
      data: { labels: revenueSeries.value.map(row => shortDate(row.date)), datasets: [{ label: 'Doanh thu', data: revenueSeries.value.map(row => Number(row.revenue)), borderColor: brand, backgroundColor: gradient, fill: true, tension: .42, pointRadius: 0, pointHoverRadius: 5, pointHoverBackgroundColor: surface, pointHoverBorderColor: brand, pointHoverBorderWidth: 3, borderWidth: 2.5 }] },
      options: { responsive: true, maintainAspectRatio: false, animation, interaction: { intersect: false, mode: 'index' }, plugins: { legend: { display: false }, tooltip: { displayColors: false, padding: 12, callbacks: { label: context => formatPrice(context.parsed.y) } } }, scales: { x: { border: { display: false }, grid: { display: false }, ticks: { color: muted, padding: 8 } }, y: { beginAtZero: true, border: { display: false }, grid: { color: border }, ticks: { color: muted, padding: 8, callback: value => `${Number(value / 1000000).toLocaleString('vi-VN')}tr` } } } },
    });
  }
  if (available('orders') && statusChartRef.value && activeOrderSeries.value.length) statusChart = new Chart(statusChartRef.value, {
    type: 'doughnut',
    data: { labels: activeOrderSeries.value.map(row => row.label), datasets: [{ data: activeOrderSeries.value.map(row => row.count), backgroundColor: activeOrderSeries.value.map(row => css(row.color)), borderColor: surface, borderWidth: 4, hoverOffset: 4 }] },
    options: { responsive: true, maintainAspectRatio: false, cutout: '72%', animation, plugins: { legend: { display: false }, tooltip: { displayColors: false, padding: 10 } } },
  });
  if (available('orders') && topProductsChartRef.value && topProducts.value.length) topProductsChart = new Chart(topProductsChartRef.value, {
    type: 'bar',
    data: { labels: topProducts.value.map(row => row.name), datasets: [{ label: 'Đã bán', data: topProducts.value.map(row => Number(row.sold)), backgroundColor: `${brand}d9`, hoverBackgroundColor: brand, borderRadius: 8, borderSkipped: false, barThickness: 10 }] },
    options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, animation, plugins: { legend: { display: false }, tooltip: { displayColors: false, padding: 10 } }, scales: { x: { beginAtZero: true, border: { display: false }, grid: { color: border }, ticks: { color: muted, precision: 0 } }, y: { border: { display: false }, grid: { display: false }, ticks: { color: muted } } } },
  });
}
  ...
  <div class="dashboard">
    ...
    <template v-else>
      ...
      <section class="dashboard-kpi-grid business-health-header" data-dashboard-section="metrics" aria-label="Chỉ số điều hành hôm nay">
        <article class="metric-card revenue-metric"><span>Doanh thu thuần hôm nay</span><strong>{{ available('financial') ? formatPrice(data.netCashRevenueToday) : 'Không khả dụng' }}</strong><small>Dữ liệu tài chính hiện tại</small></article>
        <article class="metric-card"><span>Đơn hàng hôm nay</span><strong>{{ available('orders') ? Number(data.operationalOrderCountToday).toLocaleString('vi-VN') : 'Không khả dụng' }}</strong><small>Hoạt động trong ngày</small></article>
        <article class="metric-card"><span>Đơn đang hoạt động</span><strong>{{ available('orders') ? Number(data.activeOrderCount).toLocaleString('vi-VN') : 'Không khả dụng' }}</strong><small>Cần tiếp tục theo dõi</small></article>
        <article class="metric-card"><span>Tỷ lệ hoàn thành</span><strong>{{ available('orders') ? `${Number(data.completionRateToday).toLocaleString('vi-VN', { maximumFractionDigits: 1 })}%` : 'Không khả dụng' }}</strong><small>Đơn hoàn tất hôm nay</small></article>
      </section>
      <div class="primary-operations-grid business-performance-workspace">
        <section class="panel revenue-panel" aria-labelledby="revenue-title"><header><div><h2 id="revenue-title">Doanh thu</h2><p>Đơn đã giao và thanh toán</p><div class="period-switch" aria-label="Khoảng thời gian doanh thu"><span>Hôm nay</span><strong>7 ngày</strong></div></div><strong>{{ available('financial') ? formatPrice(revenueTotal) : 'Không khả dụng' }}</strong></header><div v-if="available('financial') && revenueSeries.length" class="chart large"><canvas ref="revenueChartRef" role="img" aria-label="Biểu đồ doanh thu 7 ngày"></canvas></div><p v-else-if="available('financial')" class="empty">Chưa có dữ liệu doanh thu trong 7 ngày gần nhất.</p><p v-else class="empty">Không khả dụng</p><details v-if="available('financial') && revenueSeries.length" class="chart-data"><summary>Xem dữ liệu biểu đồ doanh thu</summary><table><thead><tr><th>Ngày</th><th>Doanh thu</th></tr></thead><tbody><tr v-for="row in revenueSeries" :key="row.date"><td>{{ row.date }}</td><td>{{ formatPrice(row.revenue) }}</td></tr></tbody></table></details></section>
      ...
      <div class="dashboard-insight-row secondary-insights-grid">
        <section class="panel status-panel" aria-labelledby="status-title"><header><div><h2 id="status-title">Trạng thái đơn hàng</h2><p>Phân bổ đơn đang hoạt động</p></div><strong>{{ available('orders') ? `${activeOrderTotal.toLocaleString('vi-VN')} đơn` : 'Không khả dụng' }}</strong></header><div v-if="available('orders') && activeOrderSeries.length" class="status-chart-shell"><div class="chart medium"><canvas ref="statusChartRef" role="img" aria-label="Biểu đồ trạng thái đơn hàng"></canvas><span class="chart-center"><strong>{{ activeOrderTotal }}</strong><small>đơn hoạt động</small></span></div><ul class="status-legend"><li v-for="row in activeOrderSeries" :key="row.key"><i :style="{ background: `var(${row.color})` }" aria-hidden="true"></i><span>{{ row.label }}</span><strong>{{ row.count }}</strong></li></ul></div><p v-else class="empty">{{ available('orders') ? 'Không có đơn đang hoạt động.' : 'Không khả dụng' }}</p><details v-if="available('orders') && activeOrderSeries.length" class="chart-data"><summary>Xem dữ liệu biểu đồ trạng thái</summary><ul><li v-for="row in activeOrderSeries" :key="row.key"><span>{{ row.label }}</span><strong>{{ row.count }}</strong></li></ul></details></section>
        <section class="panel products-panel" aria-labelledby="products-title"><header><div><h2 id="products-title">Sản phẩm bán chạy 7 ngày</h2><p>Theo số lượng đã bán</p></div></header><div v-if="available('orders') && topProducts.length" class="chart medium"><canvas ref="topProductsChartRef" role="img" aria-label="Biểu đồ sản phẩm bán chạy 7 ngày"></canvas></div><p v-else class="empty">{{ available('orders') ? 'Chưa có dữ liệu bán hàng.' : 'Không khả dụng' }}</p><details v-if="available('orders') && topProducts.length" class="chart-data"><summary>Xem dữ liệu biểu đồ sản phẩm bán chạy</summary><table><thead><tr><th>Sản phẩm</th><th>Đã bán</th></tr></thead><tbody><tr v-for="row in topProducts" :key="row.productId"><td>{{ row.name }}</td><td>{{ row.sold }}</td></tr></tbody></table></details></section>
      ...
      <section class="panel priority-workspace priority-orders-panel" aria-labelledby="priority-orders-title">
        ...
        <div v-else-if="priorityOrders.length" class="priority-order-table"><div class="priority-order-head" aria-hidden="true"><span>Đơn hàng</span><span>Khách hàng</span><span>Thời gian chờ</span><span>Tổng tiền</span><span>Trạng thái</span></div><button v-for="order in priorityOrders" :key="order.orderId" type="button" @click="router.push({ path: '/admin/orders', query: { status: 'ATTENTION', orderId: order.orderId } })"><strong>{{ order.orderCode }}</strong><span>{{ order.customerName || 'Khách hàng' }}</span><span>{{ order.waitingMinutes }} phút</span><strong>{{ formatPrice(order.finalAmount || 0) }}</strong><OrderStatusBadge :status="order.status" /></button></div>
...
.dashboard{gap:14px}.dashboard-kpi-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.dashboard-kpi-grid .metric-card,.business-health-header .revenue-metric,.business-health-header>.metric-card:not(.revenue-metric){grid-column:span 1;min-height:116px;padding:17px;border-radius:14px}.business-health-header .revenue-metric strong,.dashboard-kpi-grid .metric-card strong{font-size:clamp(20px,2vw,28px)}.dashboard-kpi-grid .metric-card small{color:var(--admin-muted);font-size:10px}.primary-operations-grid,.secondary-insights-grid{gap:10px}.dashboard-insight-row .status-panel{grid-column:span 4!important}.dashboard-insight-row .products-panel{grid-column:span 5!important}.dashboard-insight-row .stock-panel{grid-column:span 3!important}.panel{border-radius:14px;box-shadow:0 4px 18px rgba(43,48,56,.045)}.period-switch{display:inline-flex;gap:2px;margin-top:10px;padding:3px;border-radius:9px;background:var(--admin-sidebar-active)}.period-switch span,.period-switch strong{min-height:28px;padding:6px 10px;border-radius:7px;color:var(--admin-muted);font-size:11px;font-weight:600}.period-switch strong{background:var(--admin-surface);box-shadow:0 1px 4px rgba(43,48,56,.09);color:var(--admin-foreground)}.chart.large{height:250px}.status-chart-shell{display:grid;grid-template-columns:170px minmax(0,1fr);align-items:center;gap:14px;padding:16px}.status-chart-shell .chart{position:relative;height:170px;padding:0}.chart-center{position:absolute;inset:0;display:grid;place-content:center;text-align:center;pointer-events:none}.chart-center strong{font-size:22px}.chart-center small{color:var(--admin-muted);font-size:10px}.status-legend{display:grid;gap:8px;margin:0;padding:0;list-style:none}.status-legend li{display:grid;grid-template-columns:8px minmax(0,1fr) auto;align-items:center;gap:8px;font-size:11px}.status-legend i{width:8px;height:8px;border-radius:50%}.status-legend span{color:var(--admin-muted)}.priority-order-table{overflow-x:auto}.priority-order-head,.priority-order-table button{display:grid;grid-template-columns:minmax(125px,1fr) minmax(130px,1.2fr) minmax(90px,.7fr) minmax(110px,.8fr) minmax(120px,.8fr);align-items:center;gap:12px;min-width:720px;padding:12px 18px}.priority-order-head{border-bottom:1px solid var(--admin-border);background:var(--admin-surface-subtle);color:var(--admin-muted);font-size:10px;font-weight:700}.priority-order-table button{width:100%;min-height:56px;border:0;border-bottom:1px solid var(--admin-border);background:var(--admin-surface);color:var(--admin-foreground);text-align:left;cursor:pointer}.priority-order-table button:hover{background:var(--admin-surface-subtle)}.priority-order-table button:focus-visible{outline:3px solid var(--admin-brand);outline-offset:-3px}@media(max-width:900px){.dashboard-kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.dashboard-kpi-grid .metric-card,.business-health-header .revenue-metric,.business-health-header>.metric-card:not(.revenue-metric){grid-column:span 1}.status-chart-shell{grid-template-columns:150px minmax(0,1fr)}}@media(max-width:760px){.dashboard-kpi-grid{grid-template-columns:minmax(0,1fr)}.status-chart-shell{grid-template-columns:minmax(0,1fr)}.status-chart-shell .chart{height:200px}}
</style>
