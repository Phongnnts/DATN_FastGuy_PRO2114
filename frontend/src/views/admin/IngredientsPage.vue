...
function remove(id) {
  if (confirm('Xóa nguyên liệu?')) adminStore.deleteIngredient(id);
}
</script>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="i in adminStore.allIngredients" :key="i.id">
              ...
              <td>
                <button class="btn btn-sm btn-ghost" @click="openEdit(i)">
          ...
          <div class="grid-2">
            ...
            <div class="form-group">
              ...
              ><select v-model="form.unit" class="form-select">
                <option v-for="u in INGREDIENT_UNITS" :key="u" :value="u">
                  {{ u }}
                </option>
              </select>
            </div>
