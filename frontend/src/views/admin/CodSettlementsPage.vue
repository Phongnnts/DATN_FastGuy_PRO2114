...
<template>
  <main ref="pageRef" class="cod-page" tabindex="-1">
    ...
    <template v-else>
      <div v-if="!mobile" class="table-wrap"><table><thead><tr><th>Shipper</th><th>Ca</th><th>Dự kiến</th><th>Đã nộp</th><th>Trạng thái</th><th>Gửi lúc</th><th v-if="filter !== 'SUBMITTED'">Kết quả xác nhận</th><th>Thao tác</th></tr></thead><tbody><tr v-for="row in rows" :key="row.settlementId"><td><strong>{{ row.shipperName }}</strong></td><td>{{ row.shiftDate }}<small>{{ row.startTime }}–{{ row.endTime }}</small></td><td>{{ formatPrice(row.expectedAmount) }}</td><td>{{ formatPrice(row.submittedAmount) }}</td><td><span class="status" :class="row.status.toLowerCase()">{{ statusLabels[row.status] }}</span></td><td>{{ formatDate(row.submittedAt) }}</td><td v-if="filter !== 'SUBMITTED'" class="history"><strong>{{ formatPrice(row.verifiedAmount) }}</strong><small v-if="row.reason">{{ row.reason }}</small><small>{{ row.receivedByName }} · {{ formatDate(row.verifiedAt) }}</small></td><td><button v-if="canVerifySettlement(row)" type="button" class="verify-button" @click="openVerify(row, $event)">Xác nhận</button><span v-else>Đã xử lý</span></td></tr></tbody></table></div>
      <div v-else class="cards"><article v-for="row in rows" :key="row.settlementId"><div class="card-head"><strong>{{ row.shipperName }}</strong><span class="status" :class="row.status.toLowerCase()">{{ statusLabels[row.status] }}</span></div><dl><div><dt>Ca</dt><dd>{{ row.shiftDate }} · {{ row.startTime }}–{{ row.endTime }}</dd></div><div><dt>Dự kiến</dt><dd>{{ formatPrice(row.expectedAmount) }}</dd></div><div><dt>Đã nộp</dt><dd>{{ formatPrice(row.submittedAmount) }}</dd></div><div><dt>Gửi lúc</dt><dd>{{ formatDate(row.submittedAt) }}</dd></div><template v-if="row.status !== 'SUBMITTED'"><div><dt>Kiểm đếm</dt><dd>{{ formatPrice(row.verifiedAmount) }}</dd></div><div v-if="row.reason"><dt>Lý do</dt><dd>{{ row.reason }}</dd></div><div><dt>Người nhận</dt><dd>{{ row.receivedByName }} · {{ formatDate(row.verifiedAt) }}</dd></div></template></dl><button v-if="canVerifySettlement(row)" type="button" class="verify-button" @click="openVerify(row, $event)">Xác nhận bàn giao</button></article></div>
    </template>

    <div v-if="selected" class="modal-backdrop" @click.self="closeDialog"><section ref="dialogRef" class="dialog" role="dialog" aria-modal="true" aria-labelledby="verify-title" @keydown="trapFocus"><div class="dialog-head"><div><span>KIỂM ĐẾM TIỀN MẶT</span><h2 id="verify-title">Xác nhận bàn giao COD</h2></div><button type="button" aria-label="Đóng hộp thoại" :disabled="saving" @click="closeDialog"><i class="bi bi-x-lg" aria-hidden="true"></i></button></div><p>{{ selected.shipperName }} · Đã nộp <strong>{{ formatPrice(selected.submittedAmount) }}</strong></p><form @submit.prevent="verify"><label for="verify-outcome">Kết quả<select id="verify-outcome" v-model="outcome" :disabled="saving"><option value="SETTLED">Khớp</option><option value="SHORT">Thiếu tiền</option><option value="OVER">Thừa tiền</option></select></label><label for="verified-amount">Số tiền kiểm đếm<input id="verified-amount" v-model="verifiedAmount" type="number" min="0" step="0.01" required :disabled="saving" :aria-invalid="!!formError" :aria-describedby="formError ? 'verify-error' : undefined"></label><label v-if="outcome !== 'SETTLED'" for="verify-reason">Lý do<textarea id="verify-reason" v-model="reason" maxlength="500" required :disabled="saving" :aria-invalid="!!formError" :aria-describedby="formError ? 'verify-error' : undefined"></textarea></label><p v-if="conflictMessage" class="form-error" role="alert" aria-live="assertive">{{ conflictMessage }}</p><p v-if="formError" id="verify-error" class="form-error" role="alert">{{ formError }}</p><div class="dialog-actions"><button type="button" :disabled="saving" @click="closeDialog">Hủy</button><button type="submit" class="verify-button" :disabled="saving">{{ saving ? 'Đang xác nhận...' : 'Xác nhận' }}</button></div></form></section></div>
  </main>
</template>

<style scoped>
.cod-page{display:flex;flex-direction:column;gap:18px;color:var(--text-dark)}header{padding:26px;border-radius:22px;color:#fff;background:linear-gradient(125deg,#1b1714,#41271d)}header span,.dialog-head span{font-size:10px;font-weight:800;letter-spacing:.16em;color:var(--route-amber)}h1{margin:5px 0;font-size:32px}header p{color:rgba(255,255,255,.65)}.filter-tabs{display:flex;gap:8px;flex-wrap:wrap}.filter-tabs button,.error button,.dialog button,.verify-button,select,input,textarea{min-height:44px}.filter-tabs button,.error button,.dialog button{padding:0 16px;border:1px solid var(--border);border-radius:12px;background:#fff;font-weight:700}.filter-tabs button.active{color:#fff;border-color:var(--role-admin);background:var(--role-admin)}.mobile-filter{display:none}.notice,.state{padding:18px;border-radius:14px;background:#fff}.notice{color:#047857;background:#ecfdf5}.state{text-align:center}.state.error,.form-error{color:#b91c1c}.table-wrap{overflow:auto;border:1px solid var(--border);border-radius:18px;background:#fff}table{width:100%;min-width:1050px;border-collapse:collapse}th,td{padding:15px;text-align:left;border-bottom:1px solid var(--border-light)}th{font-size:11px;text-transform:uppercase;color:var(--text-mid)}td small,.history small{display:block;margin-top:4px;color:var(--text-mid)}.status{display:inline-flex;padding:6px 9px;border-radius:20px;font-size:11px;font-weight:800}.status.submitted{color:#92400e;background:#fef3c7}.status.short{color:#b91c1c;background:#fee2e2}.status.over{color:#1d4ed8;background:#dbeafe}.status.settled{color:#047857;background:#d1fae5}.verify-button{padding:0 16px;border:0;border-radius:11px;color:#fff;background:var(--role-admin);font-weight:800}.cards{display:grid;gap:12px}.cards article{padding:18px;border:1px solid var(--border);border-radius:16px;background:#fff}.card-head{display:flex;justify-content:space-between;gap:12px}.cards dl{display:grid;gap:8px}.cards dl div{display:flex;justify-content:space-between;gap:14px}.cards dt{color:var(--text-mid)}.cards dd{text-align:right}.cards .verify-button{width:100%}.modal-backdrop{position:fixed;z-index:1000;inset:0;display:grid;place-items:center;padding:18px;background:rgba(0,0,0,.55)}.dialog{width:min(520px,100%);max-height:calc(100vh - 36px);padding:24px;overflow:auto;border-radius:20px;background:#fff}.dialog-head{display:flex;justify-content:space-between;gap:16px}.dialog-head h2{margin:5px 0}.dialog-head>button{width:44px;padding:0}.dialog form,.dialog label{display:flex;flex-direction:column;gap:7px}.dialog form{gap:14px;margin-top:18px}.dialog label{font-weight:700}.dialog select,.dialog input,.dialog textarea{padding:10px 12px;border:1px solid var(--border);border-radius:10px;background:#fff}.dialog textarea{min-height:88px;resize:vertical}.dialog-actions{display:flex;justify-content:flex-end;gap:10px}.dialog-actions .verify-button{border:0;background:var(--role-admin)}
