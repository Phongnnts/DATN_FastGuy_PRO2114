...
<template>
  <main class="categories-page">
    ...
    <section class="content-card category-management-workspace" aria-labelledby="category-list-title">
      ...
      <div v-else-if="visibleCategories.length" class="table-wrapper">
        <table class="category-table"><thead><tr><th scope="col">Danh mục</th><th scope="col">Mô tả</th><th scope="col">Sản phẩm</th><th scope="col"><span class="sr-only">Thao tác</span></th></tr></thead><tbody><tr v-for="category in visibleCategories" :key="category.id"><td><div class="category-name"><span class="category-avatar"><img v-if="category.imageUrl" :src="category.imageUrl" alt="" loading="lazy"><template v-else>{{ category.name.charAt(0).toUpperCase() }}</template></span><div><strong>{{ category.name }}</strong><small>#{{ category.id }}</small></div></div></td><td class="description">{{ category.description || 'Chưa có mô tả' }}</td><td><span :class="['status-badge', Number(category.productCount) ? 'is-used' : 'is-empty']"><i class="bi" :class="Number(category.productCount) ? 'bi-check-circle' : 'bi-inbox'" aria-hidden="true"></i> {{ Number(category.productCount) ? 'Đang sử dụng' : 'Danh mục trống' }} · {{ category.productCount || 0 }} sản phẩm</span></td><td><div class="actions"><button class="icon-button" type="button" :aria-label="`Sửa ${category.name}`" :disabled="deletingId === category.id" @click="openEdit(category)"><i class="bi bi-pencil" aria-hidden="true"></i></button><button class="icon-button danger" type="button" :aria-label="`Xóa ${category.name}`" :disabled="deletingId !== null" @click="remove(category)"><span v-if="deletingId === category.id" class="button-spinner"></span><i v-else class="bi bi-trash3" aria-hidden="true"></i></button></div></td></tr></tbody></table>
      </div>
