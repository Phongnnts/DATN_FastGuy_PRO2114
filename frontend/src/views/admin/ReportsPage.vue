...
<template>
  <div class="reports-page">
    ...
    <template v-if="Object.keys(data).length">
      ...
      <section v-if="operatingProfit" class="finance-kpis" aria-label="Kết quả vận hành ước tính"><article><small>Doanh thu thuần</small><strong>{{ formatPrice(operatingProfit.netRevenue) }}</strong></article><article><small>Giá vốn</small><strong>{{ operatingProfit.cogs == null ? 'Chưa đủ dữ liệu' : formatPrice(operatingProfit.cogs) }}</strong></article><article><small>Chi phí cửa hàng</small><strong>{{ formatPrice(operatingProfit.storeExpenses) }}</strong></article><article><small>Kết quả vận hành ước tính</small><strong>{{ operatingProfit.estimatedOperatingResult == null ? 'Chưa đủ dữ liệu' : formatPrice(operatingProfit.estimatedOperatingResult) }}</strong></article><p v-if="!operatingProfit.costComplete" role="status">Chưa đầy đủ giá vốn: {{ operatingProfit.missingCostItemCount }} dòng thiếu dữ liệu, không tính thành 0.</p><p class="finance-note">Bao gồm khoản lương nhập tay. Số liệu quản trị ước tính, không phải kế toán thuế.</p></section>
