...
function exportCsv() {
  ...
  const rows = [
    ...
    ['Phí giao hàng', Number(d.shippingRevenue ?? 0)],
    ['Phí dịch vụ', Number(d.serviceFeeRevenue ?? 0)],
    ...
    ['Dòng tiền ròng', netRevenue.value],
  ...
  <main class="reports-page">
    ...
    <template v-if="Object.keys(data).length">
      ...
      <section class="charts-grid">
        <article class="chart-card"><header><div><h2>Xu hướng doanh thu</h2><p>Gross theo ngày giao; refund theo ngày xử lý; net cash là chênh lệch</p></div><i class="bi bi-graph-up"></i></header><div v-if="has('revenueByDay')" class="chart"><canvas :ref="el => refs.revenueDay.value = el"></canvas></div><div v-else class="empty"><i class="bi bi-bar-chart"></i><strong>Chưa có doanh thu theo ngày</strong><span>Không có đơn DELIVERED và PAID trong kỳ.</span></div></article>
        <article class="chart-card"><header><div><h2>Doanh thu theo tháng</h2><p>Gross, hoàn tiền và net cash theo MM/YYYY</p></div><i class="bi bi-calendar3"></i></header><div v-if="has('revenueByMonth')" class="chart"><canvas :ref="el => refs.revenueMonth.value = el"></canvas></div><div v-else class="empty"><i class="bi bi-calendar-x"></i><strong>Chưa có dữ liệu theo tháng</strong></div></article>
