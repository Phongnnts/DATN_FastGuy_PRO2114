...
const COLORS = { primary: '#2563EB', blue: '#3B82F6', green: '#0F9F8F', red: '#DC5964', yellow: '#D99A24', purple: '#7968D8', pink: '#C45A92', cyan: '#1E9DB2', gray: '#94A3B8' };
...
const selectedRange = computed(() => customActive.value ? { fromDate: customFrom.value, toDate: customTo.value } : financeRange({ period: activePreset.value }));
...
async function refreshEstimatedResult() { const id = ++requestId; financeWarning.value = ''; try { const value = await adminApi.getOperatingProfitReport(selectedRange.value); if (id === requestId) operatingProfit.value = value; } catch (error) { if (id === requestId) financeWarning.value = error?.response?.data?.message || error.message || 'Không thể tải kết quả vận hành ước tính.'; } }
...
function config(type, labels, datasets, options = {}) {
  ...
  return { type, data: { labels, datasets }, options: { responsive: true, maintainAspectRatio: false, animation: reducedMotion ? false : undefined, interaction: { intersect: false, mode: 'index' }, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => ctx.dataset.label?.includes('Doanh thu') ? `${ctx.dataset.label}: ${formatPrice(ctx.raw)}` : `${ctx.dataset.label || ''}: ${Number(ctx.raw).toLocaleString('vi-VN')}` } }, ...options.plugins }, scales: options.scales || {}, ...options.extra } };
}
...
function buildAllCharts() {
  ...
  const axis = (money = false) => ({ border: { display: false }, grid: { color: border, drawTicks: false }, ticks: { color: text, callback: money ? moneyTick : undefined, padding: 8 }, beginAtZero: true });
...
<template>
  <div class="reports-page">
    ...
    <template v-if="Object.keys(data).length">
      <section class="executive-kpis" :aria-label="`Tổng quan ${periodLabel}`"><article class="primary-kpi"><small>Doanh thu thuần</small><strong>{{ formatPrice(operatingProfit ? operatingProfit.netRevenue : netRevenue) }}</strong><span>{{ Number(data.operationalCompletedCount || 0).toLocaleString('vi-VN') }} đơn hoàn tất cùng cohort</span></article><article class="supporting-kpi"><small>Lợi nhuận gộp</small><strong>{{ data.grossProfit == null ? 'Chưa đủ dữ liệu' : formatPrice(data.grossProfit) }}</strong></article><article class="supporting-kpi"><small>Biên lợi nhuận gộp</small><strong>{{ data.grossMarginPercent == null ? '—' : `${Number(data.grossMarginPercent).toLocaleString('vi-VN', { maximumFractionDigits: 1 })}%` }}</strong></article><article class="supporting-kpi"><small>Tỷ lệ hoàn tất</small><strong>{{ completionRate }}%</strong></article></section>
      <section class="secondary-metrics" aria-label="Kết quả vận hành ước tính"><article><small>Tiền món</small><strong>{{ formatPrice(data.itemRevenue || 0) }}</strong></article><article><small>Phí giao hàng thu khách</small><strong>{{ formatPrice(data.shippingRevenue || 0) }}</strong></article><article><small>Giảm giá</small><strong>{{ formatPrice(data.discountTotal || 0) }}</strong></article><article><small>Doanh thu gộp</small><strong>{{ formatPrice(data.grossRevenue || 0) }}</strong></article><article><small>Đã hoàn tiền</small><strong>{{ formatPrice(data.refundTotal || 0) }}</strong><span>{{ Number(data.refundCount || 0).toLocaleString('vi-VN') }} đơn</span></article><article><small>Đơn hoàn tất cùng cohort</small><strong>{{ Number(data.operationalCompletedCount || 0).toLocaleString('vi-VN') }}</strong></article><article><small>Giá vốn</small><strong>{{ data.cogs == null ? 'Chưa đủ dữ liệu' : formatPrice(data.cogs) }}</strong></article><article><small>Tỷ lệ giá vốn</small><strong>{{ data.foodCostPercent == null ? '—' : `${Number(data.foodCostPercent).toLocaleString('vi-VN', { maximumFractionDigits: 1 })}%` }}</strong></article><article><small>Giá trị đơn trung bình</small><strong>{{ formatPrice(data.aov) }}</strong></article><template v-if="operatingProfit"><article><small>Giá vốn vận hành</small><strong>{{ operatingProfit.cogs == null ? 'Chưa đủ dữ liệu' : formatPrice(operatingProfit.cogs) }}</strong></article><article><small>Chi phí cửa hàng</small><strong>{{ formatPrice(operatingProfit.storeExpenses) }}</strong></article><article class="operating-result" :class="{ negative: Number(operatingProfit.estimatedOperatingResult) < 0 }"><small>Kết quả vận hành ước tính</small><strong>{{ operatingProfit.estimatedOperatingResult == null ? 'Chưa đủ dữ liệu' : formatPrice(operatingProfit.estimatedOperatingResult) }}</strong></article></template><p v-if="data.cogs == null" role="status">Chưa đủ dữ liệu giá vốn; lợi nhuận và tỷ lệ không được tính thành 0.</p><p v-if="operatingProfit && !operatingProfit.costComplete" role="status">Chưa đầy đủ giá vốn: {{ operatingProfit.missingCostItemCount }} dòng thiếu dữ liệu, không tính thành 0.</p><p v-if="operatingProfit" class="finance-note">Bao gồm khoản lương nhập tay. Số liệu quản trị ước tính, không phải kế toán thuế.</p></section>
      <section class="insight-line"><i class="bi bi-lightbulb"></i><p><strong>Tóm tắt {{ periodLabel }}:</strong> {{ Number(data.operationalOrderCount || 0).toLocaleString('vi-VN') }} đơn phát sinh, {{ Number(data.operationalCompletedCount || 0).toLocaleString('vi-VN') }} đơn trong cùng cohort đã giao thành công.<span v-if="topProduct"> Sản phẩm dẫn đầu là <b>{{ topProduct.name }}</b> với {{ topProduct.sold }} sản phẩm.</span></p></section>
      ...
      <section class="charts-grid">
        ...
        <article class="chart-card"><header><div><h2>Doanh thu theo giờ</h2><p>Khung giờ tạo doanh thu từ đơn đã giao</p></div><i class="bi bi-clock"></i></header><div v-if="has('revenueByHour')" class="chart"><canvas :ref="el => refs.revenueHour.value = el"></canvas></div><div v-else class="empty"><i class="bi bi-clock-history"></i><strong>Chưa có dữ liệu theo giờ</strong></div></article>
        <article class="chart-card"><header><div><h2>Hiệu suất theo thứ</h2><p>Đơn phát sinh và hoàn tất cùng cohort</p></div><i class="bi bi-calendar-week"></i></header><div v-if="has('performanceByWeekday')" class="chart"><canvas :ref="el => refs.weekday.value = el"></canvas></div><div v-else class="empty"><i class="bi bi-calendar-x"></i><strong>Chưa có dữ liệu theo thứ</strong></div></article>
        <article class="chart-card"><header><div><h2>Xu hướng hoàn tiền</h2><p>Sự kiện hoàn tiền theo ngày xử lý</p></div><i class="bi bi-arrow-counterclockwise"></i></header><div v-if="has('refundTrend')" class="chart"><canvas :ref="el => refs.refundTrend.value = el"></canvas></div><div v-else class="empty"><i class="bi bi-arrow-counterclockwise"></i><strong>Không phát sinh hoàn tiền</strong></div></article>
        <article class="chart-card"><header><div><h2>Lý do ngoại lệ</h2><p>Đơn hủy, giao thất bại hoặc hoàn kho</p></div><i class="bi bi-exclamation-diamond"></i></header><div v-if="has('exceptionReasons')" class="chart"><canvas :ref="el => refs.exceptionReasons.value = el"></canvas></div><div v-else class="empty"><i class="bi bi-check2-circle"></i><strong>Không có ngoại lệ trong kỳ</strong></div></article>
      </section>

      <section class="top-table mac-table" aria-labelledby="top-products-title"><header class="mac-table-toolbar"><div><h2 id="top-products-title">Chi tiết sản phẩm bán chạy</h2><p>Xếp hạng theo số lượng bán trong {{ periodLabel }}</p></div><span>{{ data.topProducts?.length || 0 }} sản phẩm</span></header><div v-if="has('topProducts')" class="table-wrapper mac-table-scroll" tabindex="0" aria-label="Bảng sản phẩm bán chạy có thể cuộn ngang"><table class="table"><thead><tr><th scope="col">Hạng</th><th scope="col">Sản phẩm</th><th scope="col" class="numeric">Số lượng bán</th><th scope="col" class="numeric">Doanh thu</th><th scope="col" class="numeric">Tỷ trọng</th></tr></thead><tbody><tr v-for="(product, index) in data.topProducts" :key="product.name"><td><b class="rank" :class="`rank-${index + 1}`">{{ index + 1 }}</b></td><th scope="row">{{ product.name }}</th><td class="numeric">{{ Number(product.sold).toLocaleString('vi-VN') }}</td><td class="numeric"><strong>{{ formatPrice(product.revenue || 0) }}</strong></td><td class="numeric">{{ data.itemRevenue ? (Number(product.revenue || 0) * 100 / data.itemRevenue).toFixed(1) : 0 }}%</td></tr></tbody></table></div><div v-else class="empty"><i class="bi bi-table"></i><strong>Chưa có dữ liệu xếp hạng</strong></div></section>
    </template>
    </section>
  </div>
</template>
...
<style scoped>
.reports-page { display: grid; gap: 20px; }.report-tabs{display:flex;gap:4px;width:max-content;max-width:100%;padding:4px;border:1px solid var(--border-light);border-radius:12px;background:#fff}.report-tabs button{min-height:40px;padding:8px 16px;border-radius:8px;color:var(--text-mid);font-weight:700}.report-tabs button.active{background:var(--admin-brand-soft);color:var(--admin-foreground)}.report-tabs button:focus-visible{outline:3px solid var(--primary);outline-offset:2px}.page-heading { align-items: flex-end; display: flex; justify-content: space-between; gap: 16px; }.page-heading h1 { font-size: 28px; margin: 2px 0 4px; }.page-heading p { color: var(--text-mid); font-size: 14px; }.eyebrow { color: var(--role-admin) !important; font-size: 11px !important; font-weight: 800; letter-spacing: .12em; text-transform: uppercase; }.head-actions { display: flex; gap: 9px; }
...
.executive-kpis { display: grid; grid-template-columns: 1.45fr repeat(3,minmax(0,1fr)); gap: 10px; }.executive-kpis article,.secondary-metrics article { min-width: 0; border: 1px solid #dce4eb; border-radius: 12px; background: #fff; }.executive-kpis article { display: grid; align-content: center; gap: 7px; min-height: 116px; padding: 18px; }.executive-kpis small,.secondary-metrics small { color: var(--text-mid); font-size: 11px; font-weight: 700; letter-spacing: .02em; }.executive-kpis strong,.secondary-metrics strong { font-variant-numeric: tabular-nums; }.primary-kpi { border-color: #16324f!important; background: #16324f!important; color: #fff; }.primary-kpi small,.primary-kpi span { color: #dbe8f3; }.primary-kpi strong { font-size: clamp(24px,2.4vw,34px); line-height: 1.05; }.primary-kpi span { font-size: 12px; }.supporting-kpi strong { color: #152536; font-size: clamp(19px,1.7vw,25px); }
.secondary-metrics { display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); gap: 10px; }.secondary-metrics article { display: grid; gap: 5px; min-height: 82px; padding: 14px 15px; }.secondary-metrics strong { color: #203040; font-size: 16px; }.secondary-metrics article>span { color: var(--text-mid); font-size: 11px; }.secondary-metrics>p { grid-column: 1/-1; margin: 0; padding: 11px 13px; border-radius: 8px; background: #fff7ed; color: #9a3412; }.secondary-metrics .finance-note { background: #f5f7f9; color: var(--text-mid); }.operating-result.negative strong { color: #b42318; }
.insight-line { align-items: center; background: #f5f9fc; border: 1px solid #dce4eb; border-radius: 10px; color: #365166; display: flex; gap: 12px; padding: 13px 16px; }.insight-line > i { color: #2563eb; font-size: 20px; }.insight-line p { font-size: 13px; margin: 0; }
.charts-grid { display: grid; gap: 14px; grid-template-columns: repeat(2,minmax(0,1fr)); }
.chart-card { min-width: 0; }.chart-card,.top-table { background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-lg); box-shadow: var(--shadow-xs); overflow: hidden; }.chart-card header,.top-table header { align-items: center; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; padding: 15px 17px; }.chart-card h2,.top-table h2 { font-size: 14px; margin: 0 0 3px; }.chart-card header p,.top-table header p { color: var(--text-mid); font-size: 11px; margin: 0; }.chart-card header > i { color: var(--role-admin); font-size: 18px; }.chart { height: 285px; padding: 14px; }.empty { align-items: center; color: var(--text-mid); display: flex; flex-direction: column; gap: 7px; justify-content: center; min-height: 285px; padding: 25px; text-align: center; }.empty i { color: var(--text-light); font-size: 32px; }.empty span { font-size: 11px; }.mac-table { border: 1px solid #dce4eb; border-radius: 12px; background: #fff; box-shadow: 0 1px 2px rgb(15 23 42 / 5%); overflow: hidden; }.mac-table-toolbar { min-height: 58px; background: #fff; border-bottom: 1px solid #e5ebf0!important; }.mac-table-toolbar>span { background: #eef4fb; border-radius: 7px; color: #365166; font-size: 11px; font-weight: 700; padding: 6px 9px; }.mac-table-scroll { overflow-x: auto; }.mac-table-scroll:focus-visible { outline: 3px solid #2563eb; outline-offset: -3px; }.mac-table .table { min-width: 680px; margin: 0; }.mac-table thead { background: #f5f7f9; }.mac-table th,.mac-table td { border-bottom: 1px solid #e8edf2; }.mac-table thead th { color: #607080; font-size: 10px; letter-spacing: .05em; text-transform: uppercase; }.mac-table tbody th { color: #203040; font-size: 13px; font-weight: 700; text-align: left; text-transform: none; }.mac-table tbody tr:hover { background: #f8fafc; }.mac-table tbody tr:last-child th,.mac-table tbody tr:last-child td { border-bottom: 0; }.mac-table th:first-child,.mac-table td:first-child { text-align: center; }.mac-table .numeric { text-align: right; font-variant-numeric: tabular-nums; }.rank { display: inline-flex; align-items: center; justify-content: center; min-width: 25px; color: #607080; font-variant-numeric: tabular-nums; }.rank-1 { color: #a16207; font-weight: 800; }.rank-2 { color: #475569; font-weight: 800; }.rank-3 { color: #b45309; font-weight: 800; }
@media (max-width: 1050px) { .executive-kpis,.secondary-metrics { grid-template-columns: repeat(2,minmax(0,1fr)); } }@media (max-width: 720px) { .page-heading { align-items: flex-start; flex-direction: column; }.filter-panel { align-items: stretch; }.custom-dates label { flex: 1; }.custom-dates input { width: 100%; }.charts-grid,.executive-kpis,.secondary-metrics { grid-template-columns: 1fr; }.presets { overflow-x: auto; }.error-banner { align-items: flex-start; flex-wrap: wrap; } }
@media (prefers-reduced-motion: reduce) { .reports-page * { scroll-behavior: auto; } }
</style>
