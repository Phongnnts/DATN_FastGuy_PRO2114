...
const sellingPrice = computed(() => Number(presentation.value?.variantPrice ?? selectedVariant.value?.price ?? 0));
const unitCost = computed(() => costComplete.value ? Number(presentation.value?.recipeCostPerServing ?? 0) : null);
const grossProfit = computed(() => unitCost.value == null ? null : sellingPrice.value - unitCost.value);
const grossMargin = computed(() => grossProfit.value == null || sellingPrice.value <= 0 ? null : grossProfit.value / sellingPrice.value * 100);
const limitingIngredient = computed(() => summaryRows.value.find((row) => row.limiting)?.name || '—');
...
<template>
  <main class="recipes-page">
    <header class="page-heading"><div><p class="eyebrow">Kho · Cấu hình bán hàng</p><h1>Công thức & định mức</h1><p>Công thức cho 1 phần, liên kết món bán với nguyên liệu để tính năng lực phục vụ và giá vốn.</p></div><button class="btn btn-outline" :disabled="loading" @click="loadBaseData">Làm mới</button></header>
    ...
    <template v-else>
      <section class="panel selector-panel recipe-context" aria-label="Chọn món và kích cỡ">
      ...
      <template v-if="selectedVariant">
        ...
        <section v-if="presentation" class="command-rail recipe-command-rail" aria-label="Tổng quan công thức"><article class="command-card recipe-capacity-command"><span>Năng lực bán</span><strong>{{ presentation.availableServings ?? '—' }} phần</strong><small>Giới hạn: {{ limitingIngredient }}</small></article><article class="command-card recipe-cost-command"><span>Giá vốn / phần</span><strong>{{ costComplete ? money(unitCost) : 'Chưa đủ dữ liệu' }}</strong><small>{{ costComplete ? `Food cost ${Number(presentation.foodCostPercent).toLocaleString('vi-VN', { maximumFractionDigits: 1 })}%` : 'Cần bổ sung giá nhập nguyên liệu' }}</small></article><article class="command-card"><span>Biên lãi gộp</span><strong>{{ costComplete ? `${grossMargin?.toLocaleString('vi-VN', { maximumFractionDigits: 1 })}%` : '—' }}</strong><small>Ước tính từ giá bán hiện tại</small></article></section>
        ...
        <form class="panel editor-form" aria-label="Công thức" @submit.prevent="requestRecipeSave"><fieldset class="editor-fields" :disabled="actionsDisabled || savingRecipe">
          ...
          <section v-if="presentation" class="summary recipe-evidence-rail" aria-label="Năng lực và chi phí"><h2>Năng lực và chi phí</h2><p v-if="inventoryMode === 'INGREDIENT' && recipeDirty" class="estimate-label">Ước tính theo bản nháp công thức</p><dl><div><dt>Có thể làm</dt><dd>{{ presentation.availableServings ?? '—' }} phần</dd></div><div><dt>Nguyên liệu giới hạn</dt><dd>{{ limitingIngredient }}</dd></div><template v-if="costComplete"><div><dt>Giá bán</dt><dd>{{ money(sellingPrice) }}</dd></div><div><dt>Giá vốn / phần</dt><dd>{{ money(unitCost) }}</dd></div><div><dt>Lãi gộp / phần</dt><dd>{{ money(grossProfit) }}</dd></div><div><dt>Food cost</dt><dd>{{ Number(presentation.foodCostPercent).toLocaleString('vi-VN', { maximumFractionDigits: 1 }) }}%</dd></div><div><dt>Biên lãi gộp</dt><dd>{{ grossMargin?.toLocaleString('vi-VN', { maximumFractionDigits: 1 }) }}%</dd></div></template><div v-else-if="presentation.costStatus === 'INCOMPLETE'" class="incomplete"><dt>Giá vốn / phần</dt><dd><strong>Chưa đủ dữ liệu</strong><span>{{ presentation.missingCostItemCount }} nguyên liệu thiếu giá: {{ (presentation.missingCostItems || []).map(item => item.name).join(', ') || 'chưa xác định' }}</span></dd></div><div v-else><dt>Chi phí</dt><dd>Không áp dụng</dd></div></dl>
