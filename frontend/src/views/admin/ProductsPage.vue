...
import { catalogAvailabilityPresentation, catalogCounts, filterProducts, paginateProducts, productTypes } from '@/utils/adminProductCatalog';
...
function catalogAvailability(product) {
  return catalogAvailabilityPresentation(product);
}
...
function capacityText(variant) { return variantCapacityPresentation(capacityFor(variant), capacities.loading); }
</script>
...
<template>
  <main class="products-page">
    ...
    <header class="catalog-header">
      <div><span class="eyebrow">VẬN HÀNH DANH MỤC</span><h1>Sản phẩm</h1><p>Quản lý món bán, giá và cấu hình kích cỡ. Tồn kho bên dưới là dữ liệu vận hành riêng, không phải trạng thái hiển thị danh mục.</p></div>
      <button class="add-product" type="button" @click="openAdd"><i class="bi bi-plus-lg" aria-hidden="true"></i> Thêm sản phẩm</button>
    </header>
    ...
    <div class="stats-grid" aria-label="Tóm tắt danh mục">
      <article class="stat stat-total"><span class="stat-icon"><i class="bi bi-box-seam" aria-hidden="true"></i></span><div><span>Tổng sản phẩm</span><strong>{{ counts.total }}</strong></div></article>
      <article class="stat stat-green"><span class="stat-icon"><i class="bi bi-eye" aria-hidden="true"></i></span><div><span>Hiển thị trong danh mục</span><strong>{{ counts.available }}</strong></div></article>
      <article class="stat stat-amber"><span class="stat-icon"><i class="bi bi-box2" aria-hidden="true"></i></span><div><span>SKU hết tồn vật lý</span><strong>{{ counts.outOfStock }}</strong></div></article>
      <article class="stat stat-red"><span class="stat-icon"><i class="bi bi-percent" aria-hidden="true"></i></span><div><span>Có giá giảm</span><strong>{{ counts.discounted }}</strong></div></article>
    </div>
    <section class="catalog-workspace">
      <div class="catalog-heading"><div><span>Không gian danh mục</span><h2>Danh sách sản phẩm</h2></div><p>{{ filtered.length }} / {{ counts.total }} sản phẩm</p></div>
      ...
      <div v-else class="table-wrapper desktop-catalog">
        <table class="table">
          <thead><tr><th>Ảnh</th><th>Sản phẩm</th><th>Danh mục</th><th>Giá gốc</th><th>Kích cỡ</th><th>Tồn kho và năng lực bán</th><th>Hiển thị trong danh mục</th><th>Thư viện</th><th>Thao tác</th></tr></thead>
          <tbody>
            <tr v-for="product in paginated" :key="product.id">
              ...
              <td><span :class="`badge badge-${catalogAvailability(product).tone}`">{{ catalogAvailability(product).label }}</span></td>
      ...
      <div v-if="!loading && !loadError && filtered.length" class="mobile-catalog" aria-label="Danh sách sản phẩm trên thiết bị di động">
        <article v-for="product in paginated" :key="product.id" class="product-mobile-card">
          ...
          <div class="mobile-product-main"><strong>{{ product.name }}</strong><small>#{{ product.id }} · {{ categoryName(product) }}</small><span>{{ formatPrice(product.basePrice) }}</span><dl class="mobile-facts"><div><dt>Hiển thị trong danh mục</dt><dd>{{ catalogAvailability(product).label }}</dd></div><div><dt>Tồn kho và năng lực bán</dt><dd>{{ stockStatusLabel(product) }}</dd></div></dl><span class="capacity-caption">Dữ liệu tồn kho theo từng kích cỡ</span><div class="variant-capacities"><div v-for="variant in product.variants || []" :key="variant.variantId"><strong>{{ variant.variantName || 'Mặc định' }}</strong><span class="capacity-label" :class="`capacity-${capacityText(variant).tone}`">{{ capacityText(variant).label }}</span><small v-if="capacityText(variant).detail">{{ capacityText(variant).detail }}</small></div></div></div>
...
<style scoped>
.stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:14px}.stat{display:flex;align-items:center;gap:14px;min-height:104px;padding:18px;border:1px solid rgba(23,23,23,.06);border-radius:18px;background:#fff;box-shadow:0 7px 25px rgba(42,28,20,.045)}.stat .stat-icon{display:grid;flex:0 0 42px;height:42px;place-items:center;border-radius:13px;color:var(--primary);background:var(--primary-50)}.stat>div{display:flex;flex-direction:column}.stat span:not(.stat-icon){color:var(--text-mid);font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.07em}.stat strong{color:var(--text);font-size:27px}.stat-green .stat-icon{color:#15803d;background:#ecfdf3}.stat-amber .stat-icon{color:#b45309;background:#fff7e6}.stat-red .stat-icon{color:#dc2626;background:#fff1f1}.products-page{color:var(--text-dark)}.catalog-header{position:relative;display:flex;align-items:end;justify-content:space-between;gap:30px;min-height:190px;margin-bottom:18px;padding:30px 34px;overflow:hidden;border-radius:26px;color:#fff;background:linear-gradient(125deg,#1b1714,#30231d 72%,#4a291d);box-shadow:0 20px 50px rgba(39,25,18,.13)}.catalog-header>div,.add-product{position:relative;z-index:1}.eyebrow{color:var(--route-amber);font-size:10px;font-weight:800;letter-spacing:.17em}.catalog-header h1{margin:9px 0 7px;font-size:clamp(29px,4vw,43px);line-height:1.05}.catalog-header p{color:rgba(255,255,255,.53);font-size:12px}.add-product{display:flex;align-items:center;gap:8px;min-height:46px;padding:10px 18px;border:0;border-radius:999px;color:#1b1714;background:var(--route-amber);font-size:12px;font-weight:800}.catalog-workspace{overflow:hidden;border:1px solid rgba(23,23,23,.06);border-radius:22px;background:#fff;box-shadow:0 10px 35px rgba(42,28,20,.055)}.catalog-heading{display:flex;align-items:end;justify-content:space-between;padding:22px 24px 15px}.catalog-heading span{color:var(--primary);font-size:9px;font-weight:800;letter-spacing:.14em}.catalog-heading h2{margin:4px 0 0;font-size:20px}.catalog-heading p{color:var(--text-light);font-size:11px}.catalog-toolbar{display:grid;grid-template-columns:minmax(220px,2fr) repeat(5,minmax(120px,1fr)) auto;gap:10px;padding:0 24px 18px}.search-box{position:relative}.search-box i{position:absolute;top:50%;left:13px;transform:translateY(-50%);color:var(--text-light)}.search-box input{padding-left:36px}.state{display:flex;min-height:280px;align-items:center;justify-content:center;flex-direction:column;gap:10px;color:var(--text-light);text-align:center}.state-error{color:var(--red-active)}.spinner{width:28px;height:28px;border:3px solid var(--border-light);border-top-color:var(--primary);border-radius:50%;animation:spin .8s linear infinite}.pagination{display:flex;align-items:center;justify-content:space-between;padding:16px 24px;color:var(--text-mid);font-size:13px}.pagination div,.row-actions,.dialog-actions{display:flex;align-items:center;gap:10px}.product-name{display:flex;flex-direction:column}.product-thumb{width:48px;height:48px;border-radius:12px;object-fit:cover}.dialog-overlay{position:fixed;z-index:1000;inset:0;display:grid;place-items:center;padding:20px;background:rgba(15,23,42,.55)}.hide-dialog{width:min(420px,100%);padding:24px;border-radius:18px;background:var(--surface);box-shadow:0 24px 70px rgba(15,23,42,.25)}.hide-dialog h2{margin:0 0 10px}.hide-dialog p{margin:0 0 22px;color:var(--text-mid)}.dialog-actions{justify-content:flex-end}.mobile-catalog{display:none}.product-mobile-card{display:grid;grid-template-columns:auto 1fr auto;gap:12px;align-items:center;padding:14px 16px;border-top:1px solid var(--border-light)}.mobile-product-main{display:flex;min-width:0;flex-direction:column;gap:3px}.mobile-product-main small,.mobile-product-main span{color:var(--text-mid);font-size:12px}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:1100px){.stats-grid{grid-template-columns:repeat(2,1fr)}.catalog-toolbar{grid-template-columns:repeat(2,1fr)}.search-box{grid-column:1/-1}}@media(max-width:700px){.desktop-catalog{display:none}.mobile-catalog{display:grid}.catalog-header{align-items:flex-start;flex-direction:column;min-height:0;padding:25px}.add-product{width:100%;justify-content:center}.stats-grid,.catalog-toolbar{grid-template-columns:1fr}.search-box{grid-column:auto}.catalog-heading,.pagination{align-items:flex-start;flex-direction:column;gap:8px}}@media(max-width:430px){.stats-grid{grid-template-columns:1fr}}@media(prefers-reduced-motion:reduce){.spinner{animation:none}}
.variant-capacities{display:grid;gap:6px;min-width:190px}.variant-capacities>div{display:grid;gap:2px;padding:7px;border:1px solid var(--border-light);border-radius:8px;background:var(--surface)}.variant-capacities strong{font-size:12px}.variant-capacities small{color:var(--text-mid);font-size:10px}.capacity-label{font-size:11px;font-weight:700}.capacity-success{color:#047857}.capacity-loading{color:var(--primary)}.capacity-warning{color:#b45309}.capacity-danger{color:#b91c1c}.capacity-secondary{color:var(--text-mid)}.mobile-product-main .variant-capacities{margin-top:7px}.catalog-toolbar input,.catalog-toolbar select,.catalog-toolbar button{min-height:40px}.icon-action{width:40px;height:40px}.mobile-facts{display:grid;gap:6px;margin:10px 0}.mobile-facts div{display:flex;justify-content:space-between;gap:12px}.mobile-facts dt{color:var(--text-mid);font-size:11px}.mobile-facts dd{margin:0;font-size:11px;font-weight:700;text-align:right}.capacity-caption{display:block;margin-top:8px;color:var(--text-mid);font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em}
</style>
