  ...
  chart = new Chart(chartRef.value, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { grid: { color: border }, ticks: { font: chartFont, color: textMid, callback: v => (v / 1000000).toFixed(0) + 'tr' }, beginAtZero: true }
      }
    }
  })
}
...
<template>
  <div>
    ...
    <div class="stat-grid">
      <div class="stat-card"><div class="stat-value" style="color:var(--primary)">{{ formatPrice(data.totalRevenue) }}</div><div class="stat-label">Tổng doanh thu</div></div>
      <div class="stat-card"><div class="stat-value" style="color:var(--primary)">{{ data.totalOrders.toLocaleString() }}</div><div class="stat-label">Tổng đơn hàng</div></div>
      <div class="stat-card"><div class="stat-value" style="color:var(--primary)">{{ formatPrice(data.revenueToday) }}</div><div class="stat-label">Doanh thu hôm nay</div></div>
      <div class="stat-card"><div class="stat-value" style="color:var(--primary)">{{ data.ordersToday }}</div><div class="stat-label">Đơn hôm nay</div></div>
    </div>
