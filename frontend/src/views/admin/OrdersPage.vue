...
<template>
  <main class="orders-page">
    ...
    <nav class="order-shortcuts" aria-label="Lối tắt vận hành đơn hàng">
      <button v-for="shortcut in ORDER_SHORTCUTS" :key="shortcut.key" type="button" :aria-pressed="activeStatus === shortcut.key" :class="['order-shortcut', `tone-${shortcut.tone}`, { active: activeStatus === shortcut.key }]" @click="selectStatus(shortcut.key)"><i :class="['bi', shortcut.icon]" aria-hidden="true"></i><span><strong>{{ shortcut.label }}</strong><small>{{ shortcut.description }}</small></span></button>
    </nav>
      ...
      <template v-else>
        <div class="desktop-order-table">
          <table class="table" aria-label="Danh sách đơn hàng">
            ...
            <tbody><tr v-for="order in paged" :key="order.orderId" :class="{ attention: order.attentionReasons?.length }">
...
.desktop-order-table{overflow-x:auto;border-top:1px solid var(--admin-border)}.desktop-order-table table{width:100%;min-width:980px;border-collapse:separate;border-spacing:0}.desktop-order-table tbody tr{height:68px;transition:background-color .15s ease}.desktop-order-table tbody tr:hover,.desktop-order-table tbody tr:focus-within{background:var(--admin-canvas)}.desktop-order-table tbody tr.attention{box-shadow:inset 3px 0 0 var(--admin-warning);background:color-mix(in srgb,var(--admin-warning) 8%,var(--admin-surface))}.table th{color:var(--text-mid);font-size:11px;letter-spacing:.03em;text-transform:uppercase}.table td{vertical-align:middle}.mobile-order-list{display:none}.table-skeleton{display:grid;gap:1px;background:var(--admin-border)}.table-skeleton span{display:block;height:68px;background:linear-gradient(90deg,var(--admin-surface),var(--admin-canvas),var(--admin-surface));background-size:200% 100%;animation:skeleton-slide 1.2s ease-in-out infinite}.refresh-status,.refresh-warning{margin:0 16px 12px;padding:9px 12px;border-radius:10px}.refresh-status{color:var(--admin-muted);background:var(--admin-canvas)}.refresh-warning{color:var(--admin-danger);background:color-mix(in srgb,var(--admin-danger) 8%,var(--admin-surface))}@keyframes skeleton-slide{50%{background-position:100% 0}}
...
@media (max-width: 640px) { .orders-header { align-items: flex-start; flex-direction: column; }.header-actions{width:100%}.header-actions .btn{flex:1}.order-shortcuts{grid-template-columns:repeat(2,minmax(0,1fr))}.order-shortcut{align-items:flex-start;flex-direction:column;min-height:116px}.stats { grid-template-columns: 1fr; }.filter-area { grid-template-columns: 1fr; }.advanced-date-filters > label { flex: 1; }.advanced-date-filters input { width: 100%; }.table-footer { align-items: flex-start; flex-direction: column; gap: 10px; } }
