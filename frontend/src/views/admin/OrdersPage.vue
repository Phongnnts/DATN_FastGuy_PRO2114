...
async function openPrimaryAction(order, event) {
  const action = inlineOrderActions(order.allowedActions)[0];
  await openOrder(order, event);
  if (action) selectDrawerAction(action.key);
}
function closeOrder() {
...
<template>
  <main class="orders-page">
    <header class="orders-header orders-command-header">
    ...
    <section class="panel queue-workspace orders-light-workspace">
      ...
      <div id="order-queue-panel" class="orders-queue" role="tabpanel" aria-label="Hàng đợi đơn hàng">
      <section ref="filterPanelRef" class="filter-toolbar orders-toolbar" tabindex="-1" aria-labelledby="order-filter-title">
      ...
      <template v-else>
        <div class="desktop-order-table">
          <table class="table" aria-label="Danh sách đơn hàng">
            <thead><tr><th>Đơn hàng</th><th>Khách hàng</th><th>Đã chờ</th><th>Sản phẩm</th><th>Tổng tiền</th><th>Thanh toán</th><th>Trạng thái</th><th>Hoàn tiền</th><th>Thao tác tiếp theo</th></tr></thead>
            <tbody><tr v-for="order in paged" :key="order.orderId" class="order-row-trigger" :class="{ attention: order.attentionReasons?.length, 'order-row-selected': selectedOrder?.orderId === order.orderId }" :data-order-id="order.orderId" tabindex="0" :aria-label="`Xem chi tiết đơn hàng ${order.orderCode}`" @click="openOrderFromRow(order, $event)" @keydown.enter="openOrderFromRow(order, $event)" @keydown.space="openOrderFromRow(order, $event)">
              <td><button class="order-link" type="button" :aria-label="`Xem chi tiết đơn hàng ${order.orderCode}`" @click="openOrder(order, $event)">{{ order.orderCode }}</button></td><td><strong>{{ order.customerName || 'Khách' }}</strong></td><td><strong class="waiting-primary">{{ order.waitingMinutes }} phút</strong><small class="waiting-secondary">{{ formatTime(order.createdAt) }} · {{ formatDate(order.createdAt) }}</small></td><td>{{ order.itemCount || 0 }} món</td><td><strong>{{ formatPrice(order.finalAmount || 0) }}</strong></td><td><span class="payment-method">{{ paymentMethodLabel(order.paymentMethod) }}</span><small :class="['payment-state', String(order.paymentStatus).toLowerCase()]">{{ paymentStatusLabel(order.paymentStatus) }}</small></td><td><OrderStatusBadge :status="order.status" /><div v-if="order.attentionReasons?.length" class="attention-reasons"><span v-for="reason in order.attentionReasons" :key="reason">{{ ATTENTION_REASON_LABELS[reason] }}</span></div></td><td><span v-if="order.refundStatus === 'REFUNDED'" class="refund-badge refund-done">Đã hoàn {{ formatPrice(order.refundAmount) }}</span><span v-else-if="order.refundStatus === 'REJECTED'" class="refund-badge refund-rejected">Đã từ chối</span><router-link v-else-if="order.refundStatus === 'PENDING'" class="refund-action" :to="{ path: '/admin/refunds', query: { status: 'PENDING' } }">Xử lý hoàn</router-link><span v-else class="muted">—</span></td><td class="next-action-cell"><button v-if="inlineOrderActions(order.allowedActions)[0]" type="button" class="btn btn-primary row-primary-action" @click="openPrimaryAction(order, $event)">{{ inlineOrderActions(order.allowedActions)[0].label }}</button><button v-else type="button" class="btn btn-outline row-detail-action" @click="openOrder(order, $event)">Xem chi tiết</button></td>
            </tr></tbody>
          </table>
        </div>
        <div class="mobile-order-list" aria-label="Danh sách đơn hàng"><article v-for="order in paged" :key="order.orderId" :class="['mobile-order-card', { attention: order.attentionReasons?.length }]"><header><button type="button" :aria-label="`Xem chi tiết đơn hàng ${order.orderCode}`" @click="openOrder(order, $event)">{{ order.orderCode }}</button><OrderStatusBadge :status="order.status" /></header><div class="mobile-order-main"><strong>{{ order.customerName || 'Khách' }}</strong><span>Đã chờ {{ order.waitingMinutes }} phút</span></div><dl><div><dt>Sản phẩm</dt><dd>{{ order.itemCount || 0 }} món</dd></div><div><dt>Tổng tiền</dt><dd>{{ formatPrice(order.finalAmount || 0) }}</dd></div><div><dt>Thanh toán</dt><dd>{{ paymentMethodLabel(order.paymentMethod) }} · {{ paymentStatusLabel(order.paymentStatus) }}</dd></div></dl><div v-if="order.attentionReasons?.length" class="attention-reasons"><span v-for="reason in order.attentionReasons" :key="reason">{{ ATTENTION_REASON_LABELS[reason] }}</span></div><button v-if="inlineOrderActions(order.allowedActions)[0]" :aria-label="`${inlineOrderActions(order.allowedActions)[0].label} cho đơn hàng ${order.orderCode}`" class="mobile-detail-action mobile-primary-action" type="button" @click="openPrimaryAction(order, $event)">{{ inlineOrderActions(order.allowedActions)[0].label }} <i class="bi bi-arrow-right" aria-hidden="true"></i></button><button v-else :aria-label="`Xem chi tiết đơn hàng ${order.orderCode}`" class="mobile-detail-action" type="button" @click="openOrder(order, $event)">Xem chi tiết <i class="bi bi-arrow-right" aria-hidden="true"></i></button></article></div>
...
.order-link,.refund-action,.row-primary-action,.row-detail-action{display:inline-flex;align-items:center;justify-content:center;min-height:40px}.order-link,.refund-action{min-width:40px}.next-action-cell{white-space:nowrap}
...
.table-footer { align-items: center; color: var(--text-mid); display: flex; font-size: 12px; justify-content: space-between; padding: 14px 16px; }.pagination { align-items: center; display: flex; gap: 10px; }.pagination button { align-items: center; border: 1px solid var(--border); border-radius: 7px; display: inline-flex; height: 40px; justify-content: center; width: 40px; }.pagination button:disabled { cursor: not-allowed; opacity: .4; }
...
.orders-light-workspace{border-radius:16px;background:var(--admin-surface);box-shadow:0 5px 20px rgba(43,48,56,.04)}.orders-light-workspace .status-segments{padding:10px 11px;background:var(--admin-surface-subtle)}.orders-light-workspace .status-segments>button,.orders-light-workspace .other-status-menu>button{min-height:38px;border-radius:10px}.orders-light-workspace .status-segments .active{color:var(--admin-foreground);background:var(--admin-sidebar-active);box-shadow:inset 0 0 0 1px var(--admin-border)}.orders-light-workspace .filter-toolbar{top:64px;padding:10px 11px}.desktop-order-table tbody tr.order-row-selected{background:#fffaf7;box-shadow:inset 3px 0 0 var(--admin-brand)}.status-semantic-chip{display:inline-flex;align-items:center;gap:6px}.status-semantic-dot{width:6px;height:6px;border-radius:50%;background:currentColor}.orders-light-workspace :deep(.status-badge)::before{display:inline-block;width:6px;height:6px;margin-right:6px;border-radius:50%;background:currentColor;content:""}.orders-light-workspace :deep(.status-badge){border-radius:9px}@media(prefers-reduced-motion:reduce){.desktop-order-table tbody tr{transition:none}.table-skeleton span{background:var(--admin-canvas);animation:none}}
.orders-page{--orders-paper:#f6f5f4;--orders-surface:#fff;--orders-ink:rgba(0,0,0,.9);--orders-muted:rgba(0,0,0,.54);--orders-border:rgba(0,0,0,.08);--brand-primary:#FF6846;--brand-primary-hover:#F85B38;--brand-primary-active:#E94F30;--brand-soft:#FFF1EC;--brand-soft-hover:#FFE8E0;--brand-border:#FFD8CC;--brand-text:#E95635;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--orders-ink);gap:20px}.orders-header h1{font-size:32px;font-weight:600;line-height:1.15;letter-spacing:-.036em}.orders-header p{color:var(--orders-muted);font-size:14px;line-height:1.5}.orders-header .btn{min-height:38px;border-color:var(--orders-border);border-radius:8px;background:var(--orders-surface);color:rgba(0,0,0,.78);box-shadow:none;font-weight:500}.orders-header .btn:last-child{border-color:var(--brand-primary);background:var(--brand-primary);color:#fff}.orders-light-workspace{border:1px solid var(--orders-border);border-radius:16px;background:var(--orders-surface);box-shadow:none}.orders-light-workspace .status-segments{gap:2px;padding:10px;border-color:var(--orders-border);background:var(--orders-surface)}.orders-light-workspace .status-segments>button,.orders-light-workspace .other-status-menu>button{min-height:36px;padding:0 12px;border-radius:8px;color:var(--orders-muted);font-size:13px;font-weight:500}.orders-light-workspace .status-segments .active{background:var(--brand-soft);box-shadow:none;color:var(--brand-primary)}.orders-light-workspace .filter-toolbar{position:relative;top:auto;z-index:4;padding:12px;border-color:var(--orders-border);background:#fcfcfb}.orders-light-workspace :is(.form-input,.form-select,.advanced-filter-trigger,.btn){min-height:40px;border-color:rgba(0,0,0,.1);border-radius:10px;box-shadow:none;font-size:13px}.orders-light-workspace .filter-toolbar>.btn-primary{border-color:var(--brand-primary);background:var(--brand-primary);color:#fff}.advanced-filter-panel{border-color:var(--orders-border);background:var(--orders-paper)}.active-filters button{border-radius:9999px}.desktop-order-table{border-color:var(--orders-border)}.desktop-order-table table{min-width:1080px}.desktop-order-table thead{position:sticky;top:0;z-index:3;background:var(--orders-paper)}.table th{height:42px;color:rgba(0,0,0,.45);font-size:11px;font-weight:600;letter-spacing:.025em}.desktop-order-table tbody tr{height:72px;box-shadow:none}.desktop-order-table .table th,.desktop-order-table .table td{padding-inline:14px}.desktop-order-table .table td{line-height:1.45}.desktop-order-table tbody tr:hover,.desktop-order-table tbody tr:focus,.desktop-order-table tbody tr:focus-within{background:#fbfaf9;box-shadow:none}.desktop-order-table tbody tr.order-row-selected{background:#fff9f5;box-shadow:inset 3px 0 #e7a88e}.desktop-order-table tbody tr.attention{background:#fffaf5;box-shadow:inset 3px 0 #d6a35b}.table td{border-color:rgba(0,0,0,.065);font-size:13px}.order-link{color:#bd5b3b;font-weight:600}.payment-method{font-size:13px;font-weight:600}.payment-state{font-size:11px}.orders-light-workspace :deep(.status-badge){border-radius:9999px;font-size:11px;font-weight:500}.row-primary-action{border-color:var(--brand-primary)!important;background:var(--brand-primary)!important;color:#fff!important}.row-detail-action{border-color:rgba(0,0,0,.1)!important;border-radius:8px!important;background:#fff!important;color:rgba(0,0,0,.78)!important}.table-footer{min-height:52px;border-top:1px solid var(--orders-border);color:rgba(0,0,0,.48)}.pagination button{border-color:rgba(0,0,0,.09);border-radius:8px;background:#fff}.state{background:var(--orders-surface)}.refresh-status{background:var(--brand-soft);color:var(--brand-primary)}.refresh-warning{background:#fff1ec;color:#9b5a43}.mobile-order-card{border-color:var(--orders-border)!important;border-radius:12px!important;box-shadow:none!important}.table-skeleton span{background:var(--orders-paper);animation:none}.orders-header .btn:last-child,.orders-light-workspace .filter-toolbar>.btn-primary,.row-primary-action{background:var(--brand-primary)!important;border-color:var(--brand-primary)!important;color:#fff!important;box-shadow:0 2px 5px rgba(255,104,70,.16),0 0 14px rgba(255,104,70,.10)!important;transition:background 150ms ease,box-shadow 150ms ease,transform 150ms ease}.orders-header .btn:last-child:hover,.orders-light-workspace .filter-toolbar>.btn-primary:hover,.row-primary-action:hover{background:var(--brand-primary-hover)!important;box-shadow:0 3px 7px rgba(255,104,70,.20),0 0 18px rgba(255,104,70,.14)!important;transform:translateY(-1px)}.orders-header .btn:last-child:active,.orders-light-workspace .filter-toolbar>.btn-primary:active,.row-primary-action:active{background:var(--brand-primary-active)!important;box-shadow:0 1px 3px rgba(255,104,70,.14)!important;transform:none}.orders-header .btn:not(:last-child):hover,.advanced-filter-trigger:hover,.row-detail-action:hover{background:#FFF7F4!important;border-color:var(--brand-border)!important;color:var(--brand-text)!important}.orders-light-workspace .status-segments .active{background:var(--brand-soft);border:1px solid #FFE0D6;color:var(--brand-text);font-weight:600}.orders-light-workspace .status-segments>button:hover,.orders-light-workspace .other-status-menu>button:hover{background:#FAFAFA;color:#3F3F46}.orders-light-workspace :is(.form-input,.form-select):focus{border-color:rgba(255,104,70,.50);box-shadow:0 0 0 3px rgba(255,104,70,.08);outline:none}.order-link{color:var(--brand-text)}.order-link:hover{color:#D94C2D}.pagination button:hover{background:#FFF3EF;color:var(--brand-text)}.waiting-primary{display:block;color:#18181B;font-size:13px;font-weight:600}.waiting-secondary{display:block;margin-top:4px;color:#A1A1AA;font-size:11px;font-weight:400;white-space:nowrap}
</style>
