...
<template>
  <main class="banner-content-workspace">
    ...
    <div v-else class="banner-grid preview-led-content-list"><article v-for="banner in banners" :key="banner.bannerId" class="banner-card"><img :src="banner.imageUrl" :alt="banner.title"><div class="banner-body"><strong>{{ banner.title }}</strong><p>{{ banner.subtitle || 'Không có mô tả' }}</p><dl class="banner-evidence"><div><dt>Hiển thị</dt><dd>{{ banner.isActive ? 'Đang hiển thị' : 'Đang ẩn' }}</dd></div><div><dt>Đích đến</dt><dd>{{ banner.link || 'Không gắn liên kết' }}</dd></div><div><dt>Thứ tự</dt><dd>{{ banner.sortOrder }}</dd></div></dl><div class="banner-actions"><label class="toggle-switch" :aria-label="`Kích hoạt ${banner.title}`"><input type="checkbox" :checked="banner.isActive" @change="toggle(banner)"><span class="toggle-slider"></span></label><button class="btn btn-sm btn-ghost" :aria-label="`Sửa ${banner.title}`" @click="edit(banner)"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-ghost text-danger" :aria-label="`Xóa ${banner.title}`" @click="remove(banner)"><i class="bi bi-trash3"></i></button></div></div></article></div>
...
<style scoped>
.banner-content-workspace{display:grid;gap:20px}.page-header p { margin:4px 0 0; color:var(--text-mid); font-size:14px; }.banner-evidence{display:grid;gap:5px;margin:10px 0 14px}.banner-evidence div{display:flex;justify-content:space-between;gap:12px}.banner-evidence dt{color:var(--admin-muted);font-size:12px}.banner-evidence dd{margin:0;font-size:12px;font-weight:700;overflow-wrap:anywhere;text-align:right}
