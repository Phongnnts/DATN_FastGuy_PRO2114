...
const schedulableUsers = computed(() => users.value.filter((user) => ['STAFF', 'SHIPPER'].includes(user.roleName) && (user.status || 'ACTIVE') === 'ACTIVE'));
const usersByRole = computed(() => ({
  STAFF: schedulableUsers.value.filter((user) => user.roleName === 'STAFF'),
  SHIPPER: schedulableUsers.value.filter((user) => user.roleName === 'SHIPPER'),
}));
const immutableAssignmentIds = computed(() => new Set(shifts.value
  .filter((shift) => shift.status !== 'SCHEDULED' || shift.checkInAt || shift.checkOutAt)
  .map((shift) => `${slotKey(shift.shiftDate, shift.shiftCode)}|${shift.userId}`)));
...
function normalizedSlots(data) { return (Array.isArray(data?.shifts) ? data.shifts : []).map(({ shiftDate, shiftCode, userId, role }) => ({ shiftDate, shiftCode, userId: Number(userId), role })).sort((a, b) => `${a.shiftDate}|${a.shiftCode}|${a.userId}|${a.role}`.localeCompare(`${b.shiftDate}|${b.shiftCode}|${b.userId}|${b.role}`)); }
function hydrate(data) {
  shifts.value = Array.isArray(data?.shifts) ? data.shifts : [];
  const grouped = {};
  shifts.value.forEach((shift) => { const key = slotKey(shift.shiftDate, shift.shiftCode); (grouped[key] ||= []).push(String(shift.userId)); });
  selections.value = grouped;
  baseline = JSON.stringify(normalizedSlots(data));
}
function selectedUserIds(date, code) { return selections.value[slotKey(date, code)] || []; }
function isImmutableAssignment(date, code, userId) { return immutableAssignmentIds.value.has(`${slotKey(date, code)}|${userId}`); }
function toggleAssignment(date, code, userId, checked) {
  const key = slotKey(date, code);
  const selected = new Set(selectedUserIds(date, code));
  if (checked) selected.add(String(userId)); else selected.delete(String(userId));
  selections.value[key] = [...selected];
}
...
async function saveWeek() {
  ...
  const slots = days.value.flatMap((day) => SHIFT_CODES.flatMap((code) => selectedUserIds(day.key, code).map((userId) => { const user = schedulableUsers.value.find((candidate) => candidate.userId === Number(userId)); return { shiftDate: day.key, shiftCode: code, userId: Number(userId), role: user.roleName }; })));
  try {
...
async function loadMonth() {
  ...
  try {
    ...
    const byId = new Map();
    results.flatMap(data => Array.isArray(data?.shifts) ? data.shifts : []).forEach(shift => byId.set(shift.shiftId || `${slotKey(shift.shiftDate, shift.shiftCode)}|${shift.userId}`, shift));
...
<template>
  <main class="shifts-page">
    <header class="page-header operations-studio-page-header"><div><h1>Ca làm nhân viên</h1><p>Lập lịch tuần và giám sát vận hành.</p></div></header>
    ...
    <section v-if="tab === 'schedule'" id="schedule-panel" class="panel schedule-workspace" role="tabpanel" aria-labelledby="schedule-tab">
      ...
      <template v-if="view === 'week'"><div v-if="loadError" class="state error" role="alert">{{ loadError }} <button class="btn btn-outline" @click="loadWeek">Thử lại</button></div><div v-else-if="loading" class="state" aria-live="polite">Đang tải lịch tuần...</div><div v-else class="week-calendar-shell week-calendar" aria-label="Lịch bảy ngày ba ca"><div class="calendar-corner"></div><div v-for="day in days" :key="`head-${day.key}`" class="calendar-head" :class="{ today: day.today }">{{ day.label }}</div><template v-for="code in SHIFT_CODES" :key="code"><div class="shift-label"><strong>{{ CODE_LABELS[code] }}</strong></div><fieldset v-for="day in days" :key="slotKey(day.key, code)" class="assignment-cell week-slot"><legend class="sr-only">{{ CODE_LABELS[code] }} {{ day.label }}</legend><details class="assignment-picker"><summary>{{ selectedUserIds(day.key, code).length ? `${selectedUserIds(day.key, code).length} người` : 'Chưa phân công' }}</summary><div class="assignment-options"><section v-for="role in ['STAFF', 'SHIPPER']" :key="role"><strong>{{ role === 'STAFF' ? 'Nhân viên cửa hàng' : 'Shipper' }}</strong><label v-for="user in usersByRole[role]" :key="user.userId"><input type="checkbox" :checked="selectedUserIds(day.key, code).includes(String(user.userId))" :disabled="isImmutableAssignment(day.key, code, user.userId)" @change="toggleAssignment(day.key, code, user.userId, $event.target.checked)"><span>{{ user.fullName }} <small v-if="isImmutableAssignment(day.key, code, user.userId)">Đã chấm công</small></span></label><small v-if="!usersByRole[role].length">Không có người khả dụng</small></section></div></details><div class="assignment-chips"><span v-for="userId in selectedUserIds(day.key, code)" :key="userId" class="assignment-chip">{{ schedulableUsers.find(user => String(user.userId) === userId)?.fullName }}<small>{{ schedulableUsers.find(user => String(user.userId) === userId)?.roleName }}</small></span></div></fieldset></template></div></template>
...
.assignment-picker{position:relative}.assignment-picker summary{display:flex;min-height:44px;align-items:center;padding:8px;border:1px solid var(--admin-hairline);border-left:3px solid var(--admin-brand);border-radius:10px;background:#fff;cursor:pointer;font-size:11px;font-weight:700}.assignment-options{width:100%;max-height:260px;overflow:auto;margin-top:4px;padding:10px;border:1px solid var(--admin-hairline);border-radius:12px;background:#fff;box-shadow:0 12px 28px rgba(24,34,48,.16)}.assignment-options section+section{margin-top:10px;padding-top:10px;border-top:1px solid var(--admin-hairline)}.assignment-options strong,.assignment-options small{display:block;margin-bottom:4px;font-size:10px}.assignment-options label{display:flex;min-height:40px;align-items:center;gap:8px;font-size:11px}.assignment-options input{width:18px;height:18px}.assignment-chips{display:grid;gap:4px;margin-top:6px}.assignment-chip{display:flex;justify-content:space-between;gap:4px;padding:4px 6px;border-radius:6px;background:var(--admin-brand-soft);font-size:10px}.assignment-chip small{font-size:8px;color:var(--admin-muted)}
</style>
