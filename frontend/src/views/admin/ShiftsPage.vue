...
function handleTabKeydown(event, index) { const keys = { ArrowLeft: index - 1, ArrowRight: index + 1, Home: 0, End: 1 }; if (!(event.key in keys)) return; event.preventDefault(); const next = (keys[event.key] + 2) % 2; selectTab(next ? 'monitoring' : 'schedule'); tabRefs.value[next]?.focus(); }
...
<template>
  <main class="shifts-page">
    ...
    <section v-else id="monitor-panel" class="panel" role="tabpanel" aria-labelledby="monitor-tab">
