...
function selectSection(section) {
  if (!section.disabled) activeSection.value = section.id;
}
...
<template>
  <main class="editor-page" :inert="confirmDialogOpen ? '' : undefined">
    ...
    <template v-else-if="loadState === 'ready' && categoryState === 'ready'">
      <div class="section-tabs" role="tablist" aria-label="Phần chỉnh sửa sản phẩm">
        <button v-for="(section, index) in sections" :id="`product-tab-${section.id}`" :key="section.id" :ref="(element) => { tabRefs[index] = element; }" role="tab" type="button" :aria-selected="activeSection === section.id" :aria-controls="`product-panel-${section.id}`" :tabindex="activeSection === section.id ? 0 : -1" :disabled="section.disabled" @click="selectSection(section)" @keydown="handleTabKeydown($event, index)">{{ section.label }}<span v-if="dirtySections[section.id]" class="dirty" aria-label="Có thay đổi">•</span></button>
      </div>
      <div class="editor-layout">
        ...
        <div class="editor-workflow">
          <ProductGeneralSection ref="generalSection" v-model="draft" :categories="adminStore.allCategories" :busy="saving" :baseline-version="baselineVersion" :external-errors="generalErrors" @save="saveProduct" @dirty-change="setSectionDirty('general', $event)" />
          <ProductMediaSection v-model="draft" :busy="saving" :baseline-version="baselineVersion" @save="saveProduct" @dirty-change="setSectionDirty('media', $event)" />
          <ProductVariantsSection v-model="draft" :busy="saving" :baseline-version="baselineVersion" :product-id="productId" :mode="isCreateMode ? 'create' : 'edit'" :pending="pendingVariants" @save="saveVariantsSection" @retry-pending="retryPendingVariants" @reload="reloadFromSection('variants')" @dirty-change="setSectionDirty('variants', $event)" @update:pending="pendingVariants = $event" />
          <ProductModifiersSection v-model="draft" :busy="saving" :baseline-version="baselineVersion" :product-id="productId" @reload="reloadFromSection('modifiers')" @dirty-change="setSectionDirty('modifiers', $event)" />
