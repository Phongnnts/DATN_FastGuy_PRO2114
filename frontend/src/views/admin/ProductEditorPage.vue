...
<template>
  <main class="editor-page" :inert="confirmDialogOpen ? '' : undefined">
    ...
    <template v-else-if="loadState === 'ready' && categoryState === 'ready'">
      ...
      <section :id="`product-panel-${activeSection}`" role="tabpanel" :aria-labelledby="`product-tab-${activeSection}`">
        <ProductGeneralSection v-if="activeSection === 'general'" ref="generalSection" v-model="draft" :categories="adminStore.allCategories" :busy="saving" :baseline-version="baselineVersion" :external-errors="generalErrors" @save="saveProduct" @dirty-change="setSectionDirty('general', $event)" />
