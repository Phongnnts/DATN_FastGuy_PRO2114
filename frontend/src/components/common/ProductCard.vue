...
<template>
  <article class="product-card" :class="{ 'list-mode': listMode }">
    ...
    <div class="product-footer">
      <div class="product-price"><template v-if="discountPrice"><span class="price-now">{{ formatPrice(discountPrice) }}</span><span class="price-old">{{ formatPrice(product.price) }}</span></template><span v-else class="price-now">{{ formatPrice(product.price) }}</span></div>
...
@media(max-width:560px){.product-footer{min-height:52px;padding:8px 9px 10px}.product-footer .option-btn{min-height:36px;padding:0 9px}.product-footer .option-btn span{display:none}}
</style>
