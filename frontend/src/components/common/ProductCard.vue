...
<template>
  <article class="product-card" :class="{ 'list-mode': listMode, 'homepage-card': homepage }">
    <router-link :to="`/product/${product.productId}`" class="product-main" :aria-label="`Xem chi tiết ${product.name}`">
      <div class="product-image"><img v-if="product.image && !imageFailed" :src="product.image" :alt="product.name" loading="lazy" decoding="async" @error="imageFailed = true"><div v-else class="image-fallback" role="img" :aria-label="`Chưa có ảnh ${product.name}`"><i class="bi bi-image"></i></div><div class="badges"><span v-if="product.bestSeller" class="best-badge">Bán chạy</span><span v-if="product.isNew" class="new-badge">Mới</span><span v-if="hasDiscount" class="hot-badge">-{{ Math.round(product.discountPercent ?? (1 - discountPrice / product.price) * 100) }}%</span><span v-if="product.isCombo" class="combo-badge">Combo</span><span v-if="product.spiceLevel > 0" class="spice-badge">Cay {{ product.spiceLevel }}/3</span><span v-if="hasOptions" class="option-badge">Tùy chọn</span></div><div v-if="!product.inStock || product.isAvailableNow === false" class="stock-badge">{{ product.isAvailableNow === false ? 'Ngoài giờ bán' : 'Hết hàng' }}</div></div>
...
@media(max-width:560px){.product-footer{min-height:60px;padding:8px 9px}.product-footer .option-btn{min-height:44px;padding:0 10px}.product-footer .option-btn span{display:none}}
